...
class AudioStreamProcessor:
    ...
    def set_config(self, config: dict):
        ...
        self._asr_interval_ms = config.get("asr_interval_ms", 500)
    ...
    async def process_audio_chunk(self, audio_data: bytes) -> dict:
        ...
        should_do_asr = (
            self._buffer_duration_ms >= self._asr_interval_ms and
            vad_result["is_speaking"]
        ) or (
            vad_result.get("state_changed") and not vad_result["is_speaking"]
        )
        
        if should_do_asr and self._asr_client:
            try:
                asr_result = await self._asr_client.recognize(bytes(self._audio_buffer))
                result["asr"] = asr_result
                
                self._audio_buffer.clear()
                self._buffer_duration_ms = 0
                self._last_asr_time = current_time
                
                if self._agent_interrupt and asr_result.get("text"):
                    is_final = not vad_result["is_speaking"]
                ...
                logger.error(f"ASR error in stream processor: {e}")
