/**
 * CXFC 客户端（迁移自宿主应用，去宿主化；通过依赖注入可在纯 node 环境单测）。
 *
 * 将本地已启动的插件运行时注册到 CX-O 后端（/cxfc/register），并按契约周期
 * 发送心跳（/cxfc/heartbeat）；后端不可用/断线时指数退避自动重连；进程退出时
 * 注销（DELETE /cxfc/plugins/{pluginId}）。
 *
 * 注册载荷从插件运行信息构建：host/port/name/version/tools/skills +
 * tls_cert_fingerprint + token（令牌由调用侧生成并持有）。
 */
import { randomBytes } from 'node:crypto';

/** 插件运行信息（由调用方从插件运行时对象读取/构造）。 */
export interface PluginRuntimeInfo {
  ...
  /** 注册令牌（调用侧持久化生成） */
  token: string;
  /** TLS 自签名证书指纹（SHA-256 十六进制冒号分隔，/health 暴露） */
...
export interface CxfcClientOptions {
   ...
   * （配置修改后端地址后无需重启）。返回空值时回落构造期 backendUrl 快照。
...
/** 单请求超时（毫秒）：防止后端挂起卡死退出链（Promise.allSettled）。 */
...
export class CxfcClient {
  ...
  /** 停止循环并注销（进程退出时调用；注销为尽力而为，失败不阻断退出）。 */
