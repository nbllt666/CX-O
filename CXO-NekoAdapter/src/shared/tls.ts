/**
 * TLS 自签名证书生成、持久化与指纹计算（迁移自宿主应用电脑控制插件模块，
 * 去宿主化；证书目录由调用方指定，适配器内位于 data/certs/ 下）。
 *
 * 使用 node-forge 纯 JS 生成 2048 位 RSA 自签名证书（serverAuth），并持久化到
 * 本地数据目录，避免每次启动重新生成导致客户端指纹漂移。指纹取证书 DER 的
 * SHA-256 摘要，供客户端校验（防中间人），也通过 /health 暴露。
 *
 * 不依赖宿主框架，可纯 node 单测。
 */
...
export function generateSelfSignedCertificate(opts: {
  ...
  const attrs = [
    { name: 'commonName', value: opts.commonName ?? 'cxo-neko-adapter-toolBridge' },
