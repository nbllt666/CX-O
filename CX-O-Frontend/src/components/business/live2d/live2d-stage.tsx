...
// Liquid Glass: data-glass 属性构建（舞台容器注入）
...
export function Live2DStage({
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-1 left-1 text-xs text-[var(--color-text-tertiary)] opacity-50 pointer-events-none select-none">
        Drag to move · Shift+Drag to scale · Mouse gaze tracking
      </div>
      <div className="absolute top-1 right-1 text-xs text-[var(--color-text-tertiary)] opacity-50 pointer-events-none select-none">
        {status}
      </div>
    </div>
  );
