/**
 * @file sidebar.tsx — Sidebar 业务组件重组（模块7）
 * ============================================================================
 * 模块: 模块7 业务组件重组层 — A 组布局类（layout 子目录）
 * 落点: C:\CX-O\CX-O-Frontend\src\components\business\layout\sidebar.tsx
 * 原组件: src/components/layout/Sidebar.tsx
 *
 * 重组策略（MODULE-7 AGENTS.md §2.4）:
 *   - 保留现有业务逻辑（navItems / Agent 加载 / chat 子菜单 / 折叠功能 全部不变）
 *   - UI 层换用模块6 ui-v2 Tooltip（position→side prop 适配）
 *   - 注入 Liquid Glass + data-glass（aside 容器挂载属性）
 *   - 保留 sidebar 专属 motion variants（宽度动画 / 子菜单展开 / 指示器 等行为动画）
 *   - 通过 className 消费 token，不硬编码颜色
 *
 * 跨模块导入约束（rules-0 §四 + MODULE-7 AGENTS.md §2.3）:
 *   - 仅 import 模块3 motion / 模块4 glass / 模块6 ui-v2 公开产出
 *   - 仅 import app 级共享基础设施（store / api / router）
...
import { Tooltip } from '@/components/ui-v2';
...
// sidebar 专属 motion variants（宽度动画 / 子菜单展开 / 指示器 等行为动画）
 ...
 * Sidebar 业务组件（重组版）。
 *
 * 业务逻辑保留: navItems / Agent 加载 / chat 子菜单 / 折叠功能 / 路由高亮 全部原样保留。
 * UI 层重组: Tooltip 换用 ui-v2 版本（position→side），aside 容器挂载 data-glass 属性。
 */
