...
export type BoneTag = { type: 'bone'; boneName: string; rotation: { x: number; y: number; z: number }; speed: number };
export type PoseTag = { type: 'pose'; durationMs: number };
export type ReleaseTag = { type: 'release' };
export type WindTag = { type: 'wind'; direction: number; strength: number; gustStrength: number; gustFrequency: number; gustDuration: number | string };
export type SleepTag = { type: 'sleep'; duration_ms: number };

export type AvatarTag = EmotionTag | BlendTag | BoneTag | PoseTag | ReleaseTag | WindTag | SleepTag;
...
const VALID_TYPES = new Set<TagType>(['emotion', 'blend', 'bone', 'pose', 'release', 'wind', 'sleep']);
  ...
  switch (type as TagType) {
    ...
    case 'wind': {
      ...
      return {
        ...
        gustDuration: typeof gustDuration === 'number' ? clamp(gustDuration, 0, Infinity) : gustDuration,
      };
    }
    case 'sleep': {
      if (params.length < 1) return null;
