 ...
 *  - voiceworkstationApi mock 为 vi.fn，歌曲历史/任务轮询可控
 */
...
// ---------------------------------------------------------------------------
// Mock: @/api/clients/voiceworkstation（CompositionPanel 经 musicListSongs/musicGetTask/musicDeleteSong）
// ---------------------------------------------------------------------------
...
describe('dispatch 层：createRestBackend（REST 冒烟）', () => {
  beforeEach(() => {
    // APP-Frontend 未在 setup 中打桩 fetch，此处为 REST 冒烟单独 stub
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(voiceworkstationApi.musicListSongs).mockResolvedValue({ songs: [] });
  vi.mocked(voiceworkstationApi.musicGetTask).mockResolvedValue({
    song_id: '',
    title: '',
    status: 'completed',
    stage: '',
  ...
  vi.mocked(voiceworkstationApi.musicDeleteSong).mockResolvedValue({ status: 'success', song_id: '' });
});
...
describe('CompositionPanel：歌曲历史', () => {
  it('歌曲列表渲染', async () => {
    vi.mocked(voiceworkstationApi.musicListSongs).mockResolvedValue({
      songs: [
        {
          song_id: 'song_a',
          title: '测试歌曲A',
  ...
  it('删除歌曲 → 调用 musicDeleteSong', async () => {
    vi.mocked(voiceworkstationApi.musicListSongs).mockResolvedValue({
      songs: [
        {
          song_id: 'song_del',
    ...
    await waitFor(() => {
      expect(voiceworkstationApi.musicDeleteSong).toHaveBeenCalledWith('song_del');
    });
  });
...
describe('CompositionPanel：version 防乱序（组件级）', () => {
  it('version 不递增的响应被丢弃，score 不更新', async () => {
    ...
    // 等待命令执行完成
    await waitFor(() => {
      expect(vi.mocked(voiceworkstationApi.musicGetTask)).toBeDefined();
    });
