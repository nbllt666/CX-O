/**
 * VRM 表情驱动：情绪 BlendShape 过渡 + 空闲微表情。
 * 行为口径对齐 CX-O-Frontend components/business/vrm/vrm-expression.ts。
 */
import { VRM, VRMExpressionPresetName } from '@pixiv/three-vrm';
import type { EmotionType, ExpressionLayer } from '../types';
...
export class VRMExpression {
  ...
  setEmotion(emotion: EmotionType | string, intensity?: number): void {
    const resolvedEmotion =
      typeof emotion === 'string' && !this.isEmotionType(emotion)
  ...
  private rebuildMixBlendShapes(): void {
    ...
    for (const layer of this.activeExpressionMix) {
      ...
      if (
        presetName &&
        em.getValue(presetName as keyof typeof VRMExpressionPresetName) !== undefined
      ) {
