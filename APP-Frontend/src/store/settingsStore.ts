/**
 * 设置状态存储：头像（Live2D/VRM）、布局、自动保存等。
 * 行为口径对齐 CX-O-Frontend src/store/settingsStore.ts，
...
import type { FrontendLimits } from '../api/types';
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      fetchLimits: async () => {
        try {
          const limits = await configApi.getLimits();
          set({ limits });
        } catch {
          // 失败保持 null，回落到各设置自带的 min/max
        }
      },
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        const hasLegacyVRMTweak =
          ...
          persistedTweak.camera.offsetZ === 2.5 &&
          persistedTweak.camera.lookAtY === 0.45 &&
          persistedTweak.light?.directionalIntensity === 2 &&
          persistedTweak.light.ambientIntensity === 1.2 &&
          persistedTweak.light.pointIntensity === 0.8;
        return {
          ...current,
          ...p,
