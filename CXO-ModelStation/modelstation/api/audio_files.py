...
提供 GET /api/audio-files/{category}/{filename}，将生成/推理/训练数据产物
以 URL 形式暴露给前端播放。category 白名单映射到受控目录，
filename 允许一层子路径（如 datasets/<speaker>/0001_ab12.wav），
解析后路径必须位于对应目录内，否则拒绝访问（防路径穿越）。

ModelStation category 白名单（change-id: split-audio-workstation-cxfc-modelstation）：
- audition:  试听推理输出目录（sovits_svc_infer 落盘，文件名 converted_<stem>_<uuid>.wav）
- datasets:  训练数据目录（data/training/sovits_svc，允许 <speaker>/<file>.wav 子路径）
...
def _category_dirs() -> dict[str, Path]:
    ...
    - audition: 试听推理输出目录（infer 直接落盘在 audition_dir 根目录）
    - datasets: 训练数据目录（允许 <speaker>/<file> 子路径）
    """
    settings = get_settings()
    return {
        "audition": Path(settings.sovits_svc.audition_dir),
        "datasets": Path(settings.sovits_svc.training_data_dir),
    }
