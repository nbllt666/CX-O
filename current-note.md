...
- **enhance-cxfc-admin-and-integrate-dream（整体）**（**已闭合**——spec 闸门 GN-004 两轮 + 实现任务 0-7 + T8 合流三重测试 + GN-004 交付前审查警示放行 + 人类 [V] 终审批准全部通过，2026-09-05；spec 目录 `.trae/specs/enhance-cxfc-admin-and-integrate-dream/`，无独立 issue 档，留痕 8 份变更文档见下）
  ...
  - 最终结果：①CXFC 数据网关：注册签发 plugin_access_token（SHA-256 落库+内存代持）+ 7 网关端点（/cxfc/memory/search|write|stats|{id}、/cxfc/physio/report|status|sleep）+ _strip_raw_hr 隐私清洗 + admin X-API-Key 旁路；②前端 CxfcPage（/cxfc，"插件与集成"分组：插件总览/工具技能/relay/网关测试器）+ cxfc.ts 7 方法；③管理接口：POST /admin/prompt/preview（readonly 零副作用哨兵预览）+ GET/PUT /admin/model-context（operator 白名单三错误码+缓存失效）+ prompt 域进 control_plane/batch/manifest；④语音延迟：core/metrics/voice_latency.py（200 轮环形缓冲吞异常 P50/P95）+ audio.py 5 事件+client.py TTFT 接线 + GET /api/stats/voice-latency + WS voice_latency 键 + Dashboard 性能卡片（15s 轮询互斥/空态/降级）；⑤dream/autonomy 彻底集成：去 embedded 插件包装（builtin 直注+db 双形态残留清理）、配置迁入 UnifiedConfig（AutonomySection/DreamSection 8 子节+幂等迁移 .migrated 留档）、lifespan shutdown 补 stop×2+flush_physio_store、前端升一级导航（实验组剩 tuner/cluster/neko/meeting）。
  - 为什么：CXFC 网关鉴权为新增机制（既有 token 是后端→插件方向）；prompt 预览复用 build_messages 单入口零改装配；集成形态推翻旧 Frozen Decision 系统能力不应出现在可"断开"的插件列表；并行批次按文件域互斥切分（i18n 归 T5 独占、ManagementLayout T2→T7 串行、config_hot_reload T3/T6 分行追加）——实测零冲突（GN-004 rubric 8 独立验证）。
  - 未闭合项（显式登记）：①**部署项**：后端需重启加载新代码（含首次启动触发 autonomy/dream 配置迁移：旧档改名 .migrated）、前端需重新构建安装包；②真机语音一轮 E2E=当前不可判定（依赖①+在线音频栈；集成级已由测试覆盖；验证入口：重启后端→语音对话一轮→仪表盘性能卡片出现非零样本）；③test_config_router.py 单独运行 11 例失败为预存在测试隔离缺陷（FakeSettings 无 executor 属性，任意先行测试后全绿，已留痕 T6 文档）；④GN-004 观察项口径勘误：T5/T7 文档文件名 20260904 前缀 vs frontmatter 2026-09-05（保持台账交叉引用不改名）、T2 文档 i18n"47 键"实为 45 叶子键；⑤历轮遗留登记项与第六~十五轮改动叠加未 commit 状态维持（提交策略由人类定）。
  - 接续入口：本轮已闭合（2026-09-05 人类终审批准）。后续若继续：重启后端验证迁移与延迟样本（验证②）→ 前端重打包 → 历轮改动分块 commit 立项；变更文档 8 份见 `.trae/documents/2026090{4,5}_模块0_*.md`。
  - 审查记录：spec 闸门 GN-004 第一轮**警示放行**（SB-1/DV-1/O-3，人类采纳修正）+ 第二轮**警示放行**（无软阻断）；T8 交付前 GN-004 **警示放行（无 SOFT_BLOCK，2026-09-05）**——10 项 rubric 全过、测试数字与日志精确吻合、隐私红线/契约纪律/并行零冲突独立验证、checklist 47 项三值判定回填；[V] 闸门2 人类裁决**批准收尾 ✅（2026-09-05）**。

- **第十五轮全面质量评估修复（整体）**（**已闭合**——基线/4 路扫描/主线程深扫/双验证员/人类修复裁决/四批修复/主线程落盘复核+伴生补齐/全量回归/GN-004 警示放行/人类 [V] 终审批准全部通过；issue `模块0-20260830-04`，变更文档 `.trae/documents/20260830_模块0_第十五轮全面质量评估修复.md`【已关闭，2026-08-30 人类终审批准】）
  - 工程过程：用户发起第十五轮 → 动态基线与 R14 收尾完全一致（后端 **4662P/0F** + 前端 tsc 双 0 错 + vitest 665/665 + Tuner 68P + VWS 425P/1S）→ 4 路并行只读扫描 subagent + 主线程定向深扫（固定体检项 `replace(tzinfo=` 0 代码级残留）收束 **30 项候选** → 2 名独立验证员交叉复核（**30 真/0 误报**，含 A2/A9/B1/B5/C4/D6 严重度与表述校准、B1 残留范围扩大发现）→ 人类裁决【**四批全修 + 删除 B4 孤儿副本**（[V]，AskUserQuestion；public/ 修改与文件删除均显式授权）】→ 4 批并行修复（G1 后端 10 项 run_io/to_thread 卸载+鉴权 / G2 前端 3 项 tick 互斥+__vrmRuntime 清理+pinned 裁剪豁免 / G3 外围 4 项 _WS_ROOT 锚定+speaker 透传+uuid 输出+训练竞态 409 / G4 契约 5 项+B4 删除+CHANGELOG 1.11.0/0.2.0）→ 主线程 43 检查点落盘复核（3 个 FAIL 均为正则换行误差实码确认在盘）+ 伴生补齐（autonomy_engine _maybe_diary 内 2 处同型 to_thread）→ 全量回归 **4662P/0F + 665/665 + 68P + 425P/1S 零回归** → GN-004 **警示放行（CAUTION-PASS，无 SOFT_BLOCK，O1/O2 文书项已同步处置）**。
  - 交接状态：**修复/回归/主线程复核/GN-004/人类 [V] 终审全部已闭合**（终审 2026-08-30 经 NotifyUser 文件批准）；本轮整体**已关闭**。
  - 最终结果：22 项修复 + B4 孤儿副本删除（SHA256 与真源核对后）+ 7 项登记不修。G1 消事件循环阻塞 10 处（config.py 持锁 RMW 提取辅助函数 9 卸载点、mcp Popen/communicate、discovery getaddrinfo、autonomy 6 处落盘、replicator 快照写、AgentContextManager 4 处、avatars、cxfc 日志、memory tags、tuner apply_adapter 补挂 verify_admin_api_key）；G2 前端竞态 3 处；G3 外围 4 处；G4 契约对齐（template_engine 悬空引用 13 处清零、settings_json/default_yaml 指针修正、radix_config 补 3 字段 MINOR、memory.pyi 三签名对齐 PATCH）。
  - 为什么：第 13/14 轮修了 decay/archive/autonomy 时区与若干阻塞点，但同型 run_io/to_thread 漏网点仍散布 10+ 处（本轮 A 批集中归零）；**同文件多 Edit 并行批产生写竞态**（3 个 subagent 独立报告，43 检查点主线程独立落盘复核 + NameError 暴露重放兜底）——同文件多编辑必须串行、批次合流后主线程独立落盘复核固化为标准步骤。
  - 未闭合项（显式登记）：①后端需重启加载新代码、前端需重新构建安装包（部署）；②登记不修 7 项（A9 节流缓解/A10 vendored/C3 C4 轮询瞬时回退/C6 C7 风格语义/D6 停止机制不可达）；③历轮遗留（tts_service GC hooks/VRM position3d/SFT 进度条/pyi s0601 整批适配【B6 已具名补修 3 处】/R1-R5）维持原状；④第六~十五轮改动叠加未 commit（提交策略由人类定）。
  - 接续入口：本轮已闭合（2026-08-30 人类终审批准）。下次轮次固定体检项：全仓 grep `replace\(tzinfo=`；同文件多编辑串行约束；批次合流后主线程独立落盘复核；非后端套件证据统一落盘。
- **第十四轮全面质量评估修复（整体）**（**已闭合**——扫描/主线程深扫/双验证员/人类修复裁决/3 项单点修复/定向验证/全量回归/GN-004 警示放行/人类 [V] 终审批准全部通过；issue `模块0-20260830-03`，变更文档 `.trae/documents/20260830_模块0_第十四轮全面质量评估修复.md`【已关闭，2026-08-30 人类终审批准】）
  - 工程过程：用户发起第十四轮 → 动态基线（后端 **4660P/2F**【git submodule gitlink `sensevoice` 裸条目暴露 sensevoice 测试 PermissionError 缺白名单】+ 前端 tsc 双 0 错 + vitest 665/665 + Tuner 68P + VWS 425P/1S）→ 4 路并行只读扫描 subagent + 主线程定向深扫（requests.post×4 卸载/裸except 0/create_task 引用跟踪/57 份 schema 可解析/time.sleep/create_task 全绿排除）收束 **5 项候选** → 2 名独立验证员交叉复核（**5 真/0 误报/无伴生**）→ 人类裁决【批准修复 1/2/3，维持登记 4/5（[V]，NotifyUser 文件批准）】→ 3 处单点修复主线程内联（archive.py 时区 astimezone 消提前 8h 归档、sensevoice 测试 except +PermissionError、autonomy_engine.py 同型加固）→ 定向验证（sensevoice 3P + archive/autonomy 355P）→ 全量回归 **4662P/0F**（179.96s，2 失败清零净增 2 零回归）→ GN-004 **警示放行（CAUTION-PASS，无 SOFT_BLOCK）**。
  - 交接状态：**修复/回归/GN-004/人类 [V] 终审全部已闭合**（终审 2026-08-30 经 NotifyUser 文件批准）；本轮整体**已关闭**。
  - 最终结果：#1 archive.py:413-418 `replace(tzinfo=utc)`→`astimezone(utc)`（对齐 decay.py G1/A1 规则，消记忆提前 8h 归档）；#2 sensevoice/tests/test_canonical_qwenaudio_links.py except 白名单 +PermissionError（基线红元凶，gitlink 裸条目场景）；#3 autonomy_engine.py:905-910 同型加固（数据源已 aware，防遗留 naive 误标）。本轮零 public/ 触碰；GN-004 独立旁证全仓 grep `replace(tzinfo=` 仅剩 3 处注释文本 0 处代码违例。
  - 为什么：时区治理规则（G1/A1）虽在 decay.py 留注释但同型违例仍散落防御分支（archive.py/autonomy_engine.py 均为 13 轮清单外漏网）；git 状态漂移（sensevoice 转 submodule gitlink）会让既往全绿测试突然变红——测试遍历 tracked 文件须容忍 PermissionError。
  - 未闭合项（显式登记）：①后端需重启加载新代码（部署）；②#4 Tuner load_config 陷阱维持登记、#5 weaviate aware UTC vs 主存 naive 本地双基准仅观察（GN-004 O2：台账 search 类型不在 rules-0 枚举，下轮统一术语）；③历轮遗留登记项（tts_service GC hooks/VRM position3d/SFT 进度条/pyi s0601/R1-R5）维持原状；④第六~十四轮改动叠加未 commit（提交策略由人类定）。
  - 接续入口：本轮已闭合（2026-08-30 人类终审批准）。下次轮次固定体检项新增：全仓 grep `replace\(tzinfo=`（GN-004 建议已纳入）；非后端套件证据统一落盘（O2 十三轮延续）；历轮改动分块 commit。
- **第十三轮全面质量评估修复（整体）**（**已闭合**——扫描/双验证员/人类裁决/4 批次修复/落盘复核/全量回归/GN-004 警示放行/人类 [V] 终审批准全部通过；issue `模块0-20260830-02`，变更文档 `.trae/documents/20260830_模块0_第十三轮全面质量评估修复.md`【已关闭，2026-08-30 人类终审批准】）
  - 工程过程：用户发起第十三轮全面质量评估（许可无限并发 subagent）→ 动态基线与十二轮收尾一致（后端 4639P/0F + 前端 tsc 0 错 + vitest 664/665 + Tuner 68P + VWS 424P/1S）→ 4 路并行只读扫描（后端核心/前端 Electron/外围子系统/契约对照+遗留复核）31 项候选 → 2 名独立验证员逐条实码复核（**31 真/0 误报**+5 伴生，合计 36 项）→ 人类裁决【**全修 36 项 + 授权 5 份 public/ 契约**（[V]，AskUserQuestion）】→ rules-6 先写文档 → 4 批并行（G1 后端/G2 契约/G3 前端/G4 外围，文件分区零冲突）→ 主线程落盘独立复核（G2/G1 报告编辑器回显不可信后 8 处关键点 grep 验证）→ 全量回归 → GN-004 **警示放行（CAUTION-PASS，无 SOFT_BLOCK）** → **[V] 人类终审批准收尾 ✅（2026-08-30）**→ 收尾闭合。
  - 交接状态：**G1/G2/G3/G4/落盘复核/全量回归/GN-004/人类终审全部已闭合**；本轮整体**已关闭**。
  - 最终结果（按簇，详见变更档第六章）：G1 后端（decay astimezone 消 8h 衰减偏移、scheduler wait_for 300s 超时解调度器永久挂起、transport 失败入队 to_thread、admin 日志读/配置写线程化、mono rounds 语义文档对齐、_ensure_agent_session 异步化+2 处同型收敛；+10 用例）；G2 契约（distillation_log/session 两份截断 schema 补全【45/45 json.load 全量复验】、memory.schema @2.1.0 source +radix_decision、mock_memory_manager_v2 @1.3.0 对齐 pyi 三分支+降序+序号不空耗、mock_decision_core @1.1.1 +D7、CHANGELOG 1.10.0；517P 契约测试）；G3 前端（PetChat 流式按 id 定位+裁剪保护消串台/吞回复、AudioPanelPage startMic 代际+closingPromise、Live2D 三参数热更新消 28 次重建风暴、useBackendFailover 在途互斥、VRMViewer blob 早退/catch 统一 revoke、resetIsGenerating 统一、reconnect 计数清零、dispatchInFlightRef、注释×2 对齐、noble 测试 vi.mock 去环境耦合【**存量失败清零 665/665**】）；G4 外围（sovits 推理/预处理 CLI 全面对齐上游实码【-m/-c/-cm+-cr 1.0/spk 字典反查/raw 暂存/产物回收】、security_utils 锚定 _WS_ROOT+根下绝对路径放行消双重校验必 500、训练超参独立 config.json 副本真实生效、CXO-Tuner/Dockerfile 新建+compose build、trl processing_class 兼容、cosyvoice 计数回收 try 扩围、trtllm 脚本旧版口径、IdleScheduler RLock、judge 上限、CI lint pin、DatasetStore 关闭防护；VWS 425P+1S/Tuner 68P）。
  - 为什么：本轮四大根因——①CLI 集成从未对照上游 argparse（VWS sovits 全链路假接线，且上游实码与假设有出入：spk 是字典、-cm 需配 -cr，按假设修会引入新缺陷）；②契约物理损坏漏检（十二轮只修一个截断文件未全量复验，教训：修一处必须全量跑同族检查）；③run_io/防御修法"清单外同型漏网"持续（三批次靠同文件全量 grep 再捕获 5 处）；④Edit 工具落盘不可信（G2 发现回显成功但磁盘批次混合+新旧横跳，改 python 直写+回读断言，**后续所有写操作必须独立回读校验**）。
  - 未闭合项（显式登记）：①后端需重启加载新代码；②前端未重打安装包；③§五登记维持（继承十二轮 9 项 + 本轮新增：tts generator 信号量 GC hooks 观察、VRM position3d 双通道待产品裁决、Tuner load_config 陷阱、SFT 进度回退、轻项 3 条）；④admin-key UI 登记项**已注销**（实码确认已存在）；⑤O2 非后端套件证据统一落盘（GN-004 延续观察，下轮落实）；⑥第六~十三轮改动叠加未 commit（提交策略由人类定）。
  - 审查记录：GN-004 **警示放行（CAUTION-PASS，无 SOFT_BLOCK，2026-08-30）**——R1-R8 全过、14 处实码抽验相符（含 2 份截断 schema json.load 独立复现）、public/ 恰为两轮授权累积 14 M 且 rules 零触碰、回归数字与落盘日志精确一致；O1（frontmatter 滞后）已即时修正、O2/O3/O4 不阻断（O3 current-note 本条目即补写动作）。
  - 接续入口：本轮已闭合。下次"继续下一轮"建议焦点：非后端套件证据统一落盘（O2）、G4 sovits 上游 CLI 真机实跑验证（本轮为静态对照+单测级）、VRM position3d 双通道产品裁决、R1-R10 历轮登记滚动复核、God 类拆分/图遍历 N+1 立项评估、历轮改动分块 commit。

- **第十二轮全面质量评估修复（整体）**（**已闭合**——扫描/交叉验证/人类裁决/4 批次修复/内联补齐/全量回归/GN-004 PASS/人类 [V] 终审批准全部通过；issue `模块0-20260830-01`，变更文档 `.trae/documents/20260830_模块0_第十二轮全面质量评估修复.md`【已关闭，2026-08-30 人类终审批准】）
  - 工程过程：用户发起第十二轮全面质量评估（许可无限并发 subagent）→ 动态基线与十一轮收尾完全一致（后端 4637P/0F + 前端 tsc 0 错 + vitest 662/663 + Tuner 68P）→ 4 路并行只读扫描（后端核心/契约对照/前端 Electron/外围子系统+历轮登记复核）30 项候选 → 2 名独立交叉验证员逐条实码复核（25 真/3 部分真校正严重度/**2 误报剔除**：agent_context_manager 非死模块、Tuner apply 占位属设计）→ 人类裁决【全修 28 项 + 授权修订 public/ 8 份契约 + 授权删 2 个 .pyc（[V]）】→ rules-6 先写文档 → 4 批并行（G1 后端 28 文件次修改 + G2 契约 + G3 前端 + G4 外围，文件分区零冲突）→ 主线程内联补齐 3 处（handlers.py 同文件另 4 处 run_io、acp ensure_session 4 处、sovits 取消 join 改 asyncio.wait 保清理必达）→ 全量回归 → GN-004 **通过（PASS）** → 人类 [V] 终审**批准（2026-08-30）**→ 收尾闭合。
  - 交接状态：**G1/G2/G3/G4/内联补齐/全量回归/GN-004/人类终审全部已闭合**；本轮整体**已闭合**。
  - 最终结果（按簇，详见变更档第六章）：G1 后端（anythingllm dmm 6 处 run_io 化、asr_interrupt 2 处、config.py settings.json RMW 四段锁、acp manager 5+4 处、ws handlers 2+4 处、chat history limit 钳制+2 新用例）；G2 契约（cluster_backup_unit +ref_audio、storage_decision +D7 且**顺带修复原文件缺 3 闭合括号无法解析**、emotion_instruction +speed/volume、memory_manager_v2.pyi @1.1.0 write_with_decision 对齐实现、agents.pyi +7 端点、chat.pyi 补 stream/limit/Request、template_engine.pyi 悬空引用修正、physio.pyi +flush、CHANGELOG 1.9.0、2 stale .pyc 已删）；G3 前端（cxfcRelay request_id 去重+deps 单例+2 用例、ChatPage onDisconnect、useDanmakuVoice 卸载代际、CORS 防重、store:save 5MB 上限、cxfc stop 竞态、PetPage 窗口尺寸单 effect、nextMessageId 单调 id、口型混音 RAF 自停）；G4 外围（draft_registry submit_draft 接真流水线+schema 惰性加载+referencing 迁移、Dockerfile.cosyvoice 开关显式比较+浮动 tag 登记、dataset_builder per-dir 锁、sovits 取消语义修正、quality_check 加固、ci.yml +test-tuner -build 占位）。
  - 为什么：本轮三大根因——①run_io 收敛的"清单外同型漏网"（三度出现，教训：收敛按文件不按条目）；②契约对账只做 pyi 未做 schema（storage_decision 根本无法被 json 解析竟未被发现）；③多窗架构的防御缺失（后端 broadcast 无目标过滤+前端无去重）。验证教训：sovits"CancelledError 裸 raise"机械执行会跳过清理，修法必须回读上下文实码。
  - 未闭合项（显式登记）：①后端需重启加载新代码；②前端未重打安装包；③pre_generated_mock/mock_memory_manager_v2.py write_with_decision 旧形状（public/ 待 s0601/授权）；④handlers/acp 的 ensure_session 直调未收敛（轻量路径，下轮评估）；⑤§五登记不修项维持（/service/config 明文回显与用户偏好一致等 9 项）；⑥历轮台账注销 5 项（R1/R5/L-10/W3/physio 节流已自然闭合）；⑦第六~十二轮改动叠加未 commit（提交策略由人类定）。
  - 审查记录：GN-004 **通过（PASS，2026-08-30）**——R1-R8 全过、15 处实码抽验相符、public/ 纪律恰为授权范围、3 观察项（O1 非后端套件日志落盘建议/O2 行号"约"字/O3 已预披露）不阻断。
  - 接续入口：本轮已闭合。遗留项按 ①~⑦ 接续；下次"继续下一轮"建议焦点：非后端套件证据统一落盘（GN-004 O1）、ensure_session 轻量直调评估、mock_memory_manager_v2 s0601、God 类拆分/图遍历 N+1 立项评估、历轮改动分块 commit。

- **第十一轮全面质量评估修复（整体）**（**已闭合**——波1/波2/G4/全量回归/GN-004/人类 [V] 终审全部通过；issue `模块0-20260829-03`，变更文档 `.trae/documents/20260829_模块0_第十一轮全面质量评估修复.md`【已关闭，2026-08-29 人类终审批准】）
  - 工程过程：用户"继续下一轮"→ 4 路并行只读扫描（第十轮修复点逐行复核/契约全量对照/VWS music+docker 深读/历轮登记滚动复核+未覆盖区新扫）→ 3 路独立交叉验证逐条实码复核（30 真/5 部分真/0 误报+伴生 9 条）→ 人类裁决【已批准全修 23 项 + 授权 G4 契约 12 份修订（[V]）】→ rules-6 先写文档 → 波1 两路并行（G1 后端 9 项 + G2 容器 11 项，全绿闭合）→ 波2（G3 脚本/acp token/改名，192 passed 闭合）→ G4 契约修订（s0601 影响面冻结 → 2 路并行执行：G4-A schema/config 3 文件 + G4-B pyi 存根 6 份/mock/测试同步，定向 97+125 passed 全绿）→ CHANGELOG v1.8.0 主线程统一落盘 → 变更档第六章回填 → 全量回归（后端 **4637 passed/0 failed** 基线 4620 净增 17 零回归【round11_backend_final.log】；前端 tsc 0 错；vitest **662/663** 唯一失败仍为 noble 存量【round11_fe_vitest_final.json】；Tuner 68 passed【round11_tuner_final.log】）→ GN-004 交付前审查**通过（PASS）**→ 人类 [V] 终审**批准（2026-08-29，NotifyUser 裁决：批准收尾）**→ 收尾闭合（变更档转已关闭、本条目转已闭合）。
  - 交接状态：**G1（9 项）/G2（11 项）/G3（3 项）/G4（11 份契约）/全量回归/GN-004 审查/人类 [V] 终审全部已闭合**；本轮整体**已闭合**。
  - 最终结果（按簇，详见变更档第六章）：G1 后端 9 项（agent_context_manager 原子写、audio.py speed 哨兵 Optional 化、上传上限统一 20MB/配置化 50MB、ref_audio 上传族 4 点【uuid 临时名/try-finally 清理/消除 60-50 双闸门】、错误文案收敛 5 处、media_type 映射补 flac/ogg/m4a、singing_engine StopIteration 防御【实际路径 VoiceWorkStation】、acp 两死 getter+空转缓存摘除、cosyvoice 容器 python-multipart 显式声明）；G2 容器 11 项（hift capture 锁+退避、流式 eager fallback 锁、compose llm 构建上下文、asr modelscope_cache 防御+HEALTHCHECK、trtllm 旧版 CLI 对齐、prefix-caching 语义反转×2、vllm 钉版本+入口、ps1 restart/参数化/stderr、gguf 清理收窄、卷初始化注释）；G3（start-all.bat PID 复用防护、acp 协议 token 鉴权+单测、ChatHistoryManager 改名消歧）；G4 契约（speech_orchestrator v2.0.0 重写为 TTSService 实际面、ref_audio_store v1.1.0 补 13 签名、multimodal_pipeline v2.0.0 删 3 幽灵方法、emotion_instruction_service v1.1.0 补字段/async/4 函数、cx_cluster/cx_admin v1.0.1 撒谎修正、memory_manager_v2 v1.0.1 签名对齐+mock v1.2.0 同步、cluster_config 补 ref_audio、computer_control 三默认值对齐 Electron、radix_config 补 6 字段、distillation_service.pyi 注释、test_orchestrator_stub 随改）。
  - 为什么：本轮最大根因是"契约撒谎"——speech_orchestrator.pyi 声明全库不存在的类、multimodal_pipeline 3 方法已内联、cluster/computer_control/radix 三契约默认值与实现漂移，auto_fill 源会填错值；修法是 s0601 影响面冻结后"契约对齐实现，实现为源真理"，MAJOR 级幽灵契约修正因零调用方等效无破坏并按 CHANGELOG 先例降级记账；并行 CHANGELOG 冲突以"执行方不写、主线程统一落盘"规避。
  - 未闭合项（显式登记，非静默）：①~~人类 [V] 终审~~（已闭合：2026-08-29 终审批准）；②schema/cluster_backup_unit.schema.json 的 unit 枚举缺 ref_audio（伴生，待后续批次）；③memory_manager_v2.pyi write_with_decision 存根漂移（待后续批次）；④agents.pyi/chat.pyi 未覆盖端点（s0201 范围）；⑤public/interface_stub/__pycache__/ stale .pyc（public/ 删除需人类授权）；⑥God 类拆分（config.py/distillation_service/handlers/audio）/R2 图遍历 N+1/Tuner idle 预检/孤儿 schema（agent/tool/error_codes/graph_entity，已标 _seedStage）消费方挂接（另立项登记，见变更档第五章）；⑦第六~十轮+本轮改动叠加未 commit（提交策略由人类定）；⑧后端需重启加载新代码。
  - 审查记录：GN-004 交付前审查**通过（PASS，2026-08-29）**——8 项 rubric 全过（R1 22 处实码抽验相符/R2 5 项独立复现/R3 死代码测试删除非误删净增 17 自洽/R4 public/ 改动恰为 v1.8.0 授权 13 份+前轮授权 7 份且 rules 零触碰/R5 裁决三处留痕/R6 文档合规/R7 信号协议/R8 登记一致），无阻断无 SOFT_BLOCK；5 条观察项（frontmatter 路径勘误✅、方案-实施口径等效披露、note 补点名✅、docker build 未实际执行属当前不可判定、笔误级）不阻断，1/3 已顺手勘误。
  - 接续入口：本轮已闭合。遗留项接续：②~⑤契约下游缺口（cluster_backup_unit 枚举/write_with_decision 存根/未覆盖端点/stale .pyc）待后续批次或 s0201；⑥God 类拆分等另立项；⑦提交策略由人类定（第六~十一轮改动叠加工作区未 commit）；⑧后端需重启加载新代码。

- **第十轮全面质量评估修复（整体）**（**已关闭**——人类终审批准收尾，2026-08-29；issue `模块0-20260829-02`，变更文档 `.trae/documents/20260829_模块0_第十轮全面质量评估修复.md`【已关闭+GN-004 警示放行记录在档（第六章）+人类 [V] 批准】；本条目为主线程统一收尾）
  - 工程过程：用户"继续下一轮"→ 动态基线（后端 4568 passed/0 failed + 前端 tsc 0 错 + vitest 661/662，与第九轮收尾一致）→ 4 路并行只读补扫（后端核心补扫+G2 异步收敛面逐行复核【GN-004 上轮声明的未验证面】/后端外围路由/前端 Electron 补扫/外围子系统+契约+登记复核，覆盖第九轮全部未读区域）→ 2 名独立交叉验证员逐条实码复核（后端 30 真/1 部分真/0 误报+7 伴生；前端外围 18 真/3 部分真/0 误报+5 伴生）→ 人类裁决【全修 50 项 + 授权修 4 份契约（5 文件）+ convert_gguf_to_hf.py 初裁诚实化后改裁真实现】→ rules-6 先写文档 → 波1 四路（G1 回归缺口/G2 memory 收敛补全/G3 鉴权簇+graph/G4 WS handlers）→ 波2 四路（G5G6 后端杂项+memory 深层/G7 前端/G8 外围+契约/GGUF 转换器真实现专项）→ 全量回归 → GN-004 警示放行（无阻断无 SOFT_BLOCK，5 观察项收尾全闭合）→ 人类终审批准。
  - 交接状态：**约 50 项修复 + 转换器真实现 + 5 份契约修订全部闭合**（新增约 52 用例：G1 3+G2 6+G3 23+G4 10+G5G6 8+G7 1+G8 1）；全量回归**已闭合**（后端 **4620 passed/0 failed**【基线 4568 净增 52 零回归，round10_backend_final.log】；前端 tsc 0 错、vitest **662/663**【净增 1 全过，唯一失败仍为 noble 存量，round10_fe_vitest_final.json】；Tuner 68 passed；GGUF 转换器 6/6【独立复跑复现】）；**GN-004 交付前审查已闭合——警示放行（CAUTION-PASS，2026-08-29）**：8 项 rubric 中 R1（15 处抽验全属实）/R2（证据链独立复现）/R3（覆盖防丢失抽验 5/5）/R4（public/ 与 git status 逐一吻合）/R6/R7/R8（403 口径三方一致）通过，R5 警示（裁决记录未落盘→已补录第五章）；**[V] 人类终局裁决：批准收尾 ✅（2026-08-29）**；上轮教训落地：本轮全程"Edit 追加禁 Write 覆盖+同文件串行+编辑后读回"，git 用例计数核对 27 文件只增不减，零覆盖丢失。
  - 最终结果（按簇）：**第九轮 G2 回归与收敛缺口**（anythingllm stream-chat 补 await【第九轮引入的必 500 回归】、multimodal to_thread、context.py 84 次重复 import 收敛、agents stats run_io、memory.py 补 16 处 run_io 化【现 32 处】+2 处 conn.close 消除【M-D3】、archive.py 2 处、handlers/memory search 切 async 变体）；**graph 簇**（export graphml/dot 任意路径写修复：_EXPORT_DIR+is_relative_to+鉴权、31 处 run_io 簇级收敛【实码确证 threading.local 每线程连接安全】）；**鉴权漏挂簇补全**（tools PATCH+tuner trigger/feedback/delete+vector rebuild/delete/sync+acp resources/send+dream 六写路径，403 口径对齐项目，未带密钥写调用 200→403 属预期收紧）；**WS handlers 加固**（config.get 递归脱敏 api_key/secret/token、config.set Pydantic 真校验【model_validate(model_dump())，实测同类型实例校验为伪校验】+模块级写锁、dream confirm/reject to_thread、metrics/system run_io、coordinator 优先 async、gateway live bytes 分支逐消息隔离不断连）；**memory 深层**（archiver 三方法两阶段重构：压缩收集与事务写段分离、段内零 await，消除跨 await 事务污染；acp/cxfc 发现接口阻塞收包 to_thread 化【5s/2s 冻结消除】；join_group 锁内复合方法 try_add_group_member 消 TOCTOU）；**前端/Electron**（neko toolBridge try/catch 防主进程崩溃、AudioPanelPage startingRef 防麦克风流泄漏+await close、vrmWindField three-vrm 3.x 双形态、vrmEngine RAF 幂等、VideoClipBuilder tracks stop+onerror、auth 重放账本强制驱逐、memories 搜索透传过滤、LiveConsole 改连接时长卡片、ArchivePage 错误反馈）；**外围子系统**（cosyvoice 流式并发模型态竞写加锁+_active_count 计数、streaming_engine 三模型 startup 预载+加载锁【消除首连数十秒冻结】、train_job 先校验后赋值、tuner lifespan 补 DatasetStore.close、quality_check RIFF chunk 表解析）；**GGUF 转换器真实现**（convert_gguf_to_hf.py 553 行完整重写：25 条架构映射规则/24 种量化格式反量化【环境 gguf 0.19.0 实测含全部 K-quant】/两遍流式分片/HF 标准 index/诚实边界 Q8_1·Q8_K·Q1_0 跳过/tokenizer 不重建明确警告，6 用例全过）；**契约修订**（已授权：memory.pyi @1.1.0 五处签名对齐、message/api_response schema @oneOf→anyOf、agent.schema @1.1.0 源真理路径修正、cxo_tuner.pyi @1.1.0 补 /judge/build）；**第九轮遗留缺口闭合**（physio store flush 接线 autonomy/main.py 装配异常关闭路径）。
  - 为什么：本轮最大根因是"逐轮补漏"模式——第九轮 G2 按清单收敛 14 处但同文件另有 16 处未在清单内，且批量改造引入 1 处缺 await 必 500 回归；修法是把"清单式收敛"升级为"同文件全量 grep 收敛"（memory.py 现 32 处 run_io、graph.py 31 处一次到位）；鉴权漏挂簇的根因是历史端点先于鉴权口径存在，补挂时以同文件既有端点为对照逐个核对；graph.py 簇级 run_io 前先实码确证 Database 为 threading.local 每线程连接，避免盲目 to_thread 引入 sqlite 线程安全回归；archiver 跨 await 事务污染的根因是 thread_id 共享连接上隐式事务跨 await 窗口，两阶段重构让事务段封闭且无 await；GGUF 转换器按"以环境实际能力为准"原则先探测 gguf 0.19.0 的 dequantize 覆盖面再定诚实边界，避免编造不存在的支持。
  - 未闭合项（显式登记，非静默）：R3 双流 session_id 共享（产品语义）；R2 图遍历 N+1/louvain 全量重算（另立项）；T-01~T-03 God 类拆分与 T-06 原子写收敛【已扩散至 10+ 处，以 _atomic_io 为基线】（后续排期）；T-09 acp 两 getter；L-10 同名 ContextManager；L4 /ws token 与局域网形态（SEC 簇）；/acp/receive 开放协议入口（登记）；P-06 docker/asr/Dockerfile 构建上下文断裂（登记，compose 预载镜像规避）；FE10-05 vrmEngine 幂等（已防御性修复）；第六+七+八+九+十轮改动叠加在工作区未 commit（提交策略由人类定）；后端需重启加载新代码；GGUF 转换器需带 gguf 的 Python 环境（本机 cxa311）运行。
  - 接续入口：后端重启加载新代码；下次"继续下一轮"建议扫描焦点：G7/G8 外围修复点逐行独立复核（GN-004 声明的未验证面）、剩余 16 份 pyi 与 33 个 schema 的契约对照（本轮仅抽验）、VWS music/ 深读与 docker/llm 5 个 Dockerfile、R1-R10 历轮登记项滚动复核、God 类拆分与原子写收敛立项评估；提交建议按"后端核心/鉴权簇/前端/外围与docker/契约"分块 commit。

- **第九轮全面质量评估修复（整体）**（**已关闭**——人类终审批准收尾，2026-08-29；issue `模块0-20260829-01`，变更文档 `.trae/documents/20260829_模块0_第九轮全面质量评估修复.md`【已关闭+GN-004 警示放行记录在档（第六章）+人类 [V] 批准】；本条目为主线程统一收尾）
  - 工程过程：用户发起第九轮全面质量评估（性能/安全/兼容/扩展性/设计/Bug 全维度，许可无限并发 subagent）→ 动态基线（后端 4480 passed/0 failed + 前端 tsc 0 错 + vitest 656/657，与第八轮收尾一致）→ 4 路并行只读扫描（后端核心/语音实时链路/前端 Electron/设计技术债+遗留复核）定位约 50 项候选 → 2 名独立交叉验证员逐条实码复核（后端 20 真/4 部分真/0 误报+4 伴生；前端设计 19 真/1 部分真/0 误报+5 伴生；遗留 R1-R5 及第八轮登记项全部属实）→ 人类裁决【全修 34 项 + 授权删除 5 死文件 + 补 admin-key 设置页录入框 + 授权修两份 pyi】→ rules-6 先写文档 → 波1 五路并行（G1 后端核心/G2 路由层收敛/G3 WS与live/G4 集群资源/G5 前端 Electron）→ 波2 G6 技术债遗留 → 合流期追加修复 3 项（ref_audio 竞态补全/admin import+测试替身/测试覆盖恢复）→ 全量回归（两轮迭代后 4568 passed/0 failed）→ GN-004 警示放行（SOFT_BLOCK 已勘误闭合）→ 人类终审批准。
  - 交接状态：**34+3 项修复已闭合 + 测试覆盖恢复闭合**（后端全量 **4568 passed/0 failed**【基线 4480 净增 88 零回归，日志 round9_backend_final5.log】；前端 tsc 0 错、vitest **661/662**【净增 5 全过，唯一失败仍为 noble collector 存量，机器可读证据 round9_fe_vitest_final.json；round9_g5_vitest.json 同值】；CXO-Tuner 68 passed）；**GN-004 交付前审查已闭合——警示放行（CAUTION-PASS，2026-08-29）**：8 项 rubric 中 R1（14 处实码抽验全属实）/R4（public/ 仅两份授权 pyi、rules 零触碰）/R5（5删3留核实）/R6/R7 通过，R2/R3 警示（数字口径勘误已闭合：恢复验证四文件合计 163=115 三恢复文件+48 test_live_client；净增 88 已精确化）；**[V] 人类终局裁决：批准收尾 ✅（2026-08-29）**；收尾动作：中间日志 base/final/final2/final3/final4 已清理（保留 final5 为最终证据）。
  - 最终结果（按簇）：**事件循环阻塞族**（蒸馏 _run_preread to_thread+TimeoutError 补捕、WS 聊天 handlers/chat 5 处与语音 _record_dual_stream_context 3 处异步化、context 路由 11 处、memory 路由 14 处、archive/anythingllm/admin 多处 run_io 化、live_client 上下文写入异步化、chat 路由 get_agent_config_async）；**WS 重连串扰**（manager 全局单调代际 _generation_seq 防 ABA、connect 关旧 socket、disconnect 代际校验防误删新会话、broadcast/超时清理携代际、二进制帧限频 error 帧+last_activity 刷新）；**live 打断链路**（_announce_pipeline 置位/复位 set_tts_playing 接通死路径、判定 create_task 后台化+信号量+首次/二次代际校验、字幕播报独立任务不再占 _reply_task 守卫）；**agents 一致性**（stats 改 get_sessions+metadata.agent_id 过滤不再谎报、五端点锁化走 agent_store.update_agents）；**集群与资源**（heartbeat gossip 并发 gather+总超时+确认后台化去重、transport flush to_thread+枚举定位、asr _peek_stream_state 只读不落册、qwen3 _trim_tail_silence struct.unpack 向量化、ref_audio delete/set_for_agent 双侧锁内校验、vision 503 回滚限流记账、admin dashboard/stats run_io+batch 委托 AdminBatchExecutor）；**启动健壮性**（EXECUTOR 坏环境变量 log+continue+补 danmaku/interrupt_concurrency）；**前端/Electron**（CORS 透传优先不覆盖+补 x-api-key、window:move isFinite、VRM blob revoke+settingsStore merge 消毒、AudioTestPage/SummaryModal/SaveControl 清理、MeetingRoomView busy 守卫、两处 URL 编码、CompositionPanel i18n、neko directRequest 同构防护、AdminKeySection 管理密钥设置卡片+i18n 双语）；**技术债与遗留**（_pagination.clamp_pagination 收敛 6 处统一口径、Tuner docstring 更新、compose 注释对齐、archiver BEGIN IMMEDIATE 原子合并、SessionStore 内联 last_accessed 消写放大、physio 30s 节流+flush、stop-all.bat 进程名白名单+LF→CRLF 修复、lifespan 接线 session cleanup）；**契约与配置**（qwen3_tts_provider.pyi/decision_core.pyi @version 1.0.1 对齐实现【人类授权】、删除 5 个零消费死文件【人类授权：根 config/settings.json+4 yaml】）。
  - 为什么：事件循环阻塞是本轮最大面积根因（async 端点直调同步 sqlite，`*_async`/run_io 早已存在未用，系统性一次收敛）；WS 重连串扰根因是"连接身份"无代际概念，旧端点 finally 必然误删新会话，全局单调代序防 ABA 复用；live 打断死路径与热路径阻塞是一体两面（_tts_playing 恒 False 使分支不可达，接通后必须同时移出热路径）；测试覆盖丢失根因是 subagent 把既有文件当"新建"整体覆盖且定向绿测掩盖（丢失用例根本不参与运行），恢复后把"新建 vs 追加"与 git HEAD 用例计数核对列为收尾必检项；并行批次同文件编辑竞态在本轮 3 次实际发生（agents/admin import 丢失、测试替身回滚），后续多批次修复必须严格文件分区并在合流时做 import/完整性核验。
  - 未闭合项（显式登记，非静默）：SEC 安全簇 L4 /ws token 未校验延续登记（局域网部署形态）；R3 双流 session_id=agent-{agent_id} 共享上下文（产品语义裁决）；R2 图遍历 N+1/louvain 全量重算（另立项）；T-01~T-03 God 类拆分与 T-06 原子写三处收敛（后续排期）；T-09 acp 两 getter 死代码/T-11 modules 目录漂移/L-10 同名 ContextManager（观察登记）；第六+七+八+九轮改动叠加在工作区未 commit（提交策略由人类定）；后端需重启加载新代码。
  - 接续入口：后端重启加载新代码；下次"继续下一轮"建议扫描焦点：G2 异步收敛路由的逐行独立复核（GN-004 声明的未验证面）、R1-R9 历轮登记项滚动复核、s0601 契约批次、God 类拆分立项评估；提交建议按"后端核心/语音WS/前端/技术债/契约与配置"分块 commit。

- **第八轮全面质量评估修复（整体）**（**已关闭**——人类终审批准收尾，2026-08-28；issue `模块0-20260828-04`，变更文档 `.trae/documents/20260828_模块0_第八轮全面质量评估修复.md`【已关闭+GN-004 警示放行记录在档+人类 [V] 批准】；本条目为主线程统一收尾）
  - 工程过程：用户"继续下一轮"→ 动态基线（后端 4459 passed/0 failed + 前端 tsc 0 错 + vitest 647/648，与第七轮收尾完全一致）→ 4 路 search 定位 + 2 路 general_purpose_task 深扫（后端 10 项 + 前端外围 7 项）→ 2 名独立交叉验证员逐条实码复核（后端 10 真/0 误报+3 伴生；前端 5 真/2 部分真/0 误报+3 伴生）→ 用户裁决【选项 B 全修：16 项（高4中5低8 中 R8-06 登记不修实修 15）】→ rules-6 先写文档 → 批次1（G1 autonomy 持久化簇 + G2 audio/memory 杂项簇）+ 批次2（G4 前端簇 + G5 Electron/脚本簇）+ 主线程补漏（dream/physio 同类写点）→ 全量回归 → GN-004 审查。
  - 交接状态：**16+1 项修复已闭合**（新增 30 例单测：后端 21【G1 14 + G2 7】+ 前端 9【ErrorBoundary 2 + MusicPanel 7】）；全量回归**已闭合**（后端 **4480 passed/0 failed**【基线 4459 净增 21 零回归，日志 round8_backend_final.log】；前端 tsc 双 config 0 错误、vitest **656/657**【净增 9 全过，唯一失败仍为既有 noble collector 存量，机器可读证据 round8_g4_vitest.json + 基线 round8_fe_vitest_base.json】）；**GN-004 交付前审查已闭合——警示放行（CAUTION-PASS，无 SOFT_BLOCK，2026-08-28）**：八维度 rubric 7 通过+1 行号警示（3 观察项 O1/O2/O3 均不阻断且已在变更文档勘误）、独立复跑新增 21 例全过、抽验 20 处源码全属实、public/ 与 .trae/rules/ 零触碰、3 份测试证据相互印证；**[V] 人类终局裁决：批准收尾 ✅（2026-08-28）**。
  - 最终结果：持久化原子性（autonomy 域 7 处裸 JSON 写收敛 _atomic_io.py 原子写【mkstemp+fsync+os.replace】+ 双 config load 坏档回退默认+.corrupt 留痕 + physio/store 等 4 处状态文件原子化）、并发正确性（routers/autonomy 配置写 threading.Lock 串行化+_persist_enabled 改 to_thread 消除事件循环阻塞写；dream/physio update_config RMW 全程持 CONFIG_WRITE_LOCK+to_thread【主线程补漏，G1 发现项闭环】；audio.py _cancel_stale_pipeline 消除 prefill 窗口双 pipeline 并发推流）、状态持久化断链（TokenLedger 轮末持久化+KillSwitch 4 状态变更方法接线 _persist——重启不再绕过预算熔断）、事件循环阻塞（decay_batch 4 处 sync SQLite 调用 to_thread）、杂项（蒸馏空 choices IndexError 双保险+补捕、ContextManager OrderedDict LRU 256、5 处分页 limit≤200/offset≥0 钳制）、前端健壮性（全局 ErrorBoundary 根级包裹、MusicPanel 标题注入 score+空乐谱本地阻断+成功才清输入+3s 终态轮询+状态 i18n、neko.ts 成功分支 JSON 防护）、外围（Electron model:read-file 白名单=最近 pick+历史授权目录集合持久化【实码核查后方案变体，保 settingsStore 恢复与多 Agent 绑定】、stop-all.bat 新建+start-all.bat 3 处超时路径补记 PID+不再启动即清 PID 档）；修改面后端 16 文件+前端 6+electron 1+脚本 2，新建 _atomic_io.py/ErrorBoundary.tsx/stop-all.bat/test_autonomy_persistence.py/test_pagination_clamp.py/ErrorBoundary.test.tsx/MusicPanel.test.tsx。
  - 为什么：R1-R10 根因是 autonomy 域系统性遗漏原子写与 load 容错（对齐 server/config.py:1098 既有范式一次收敛）；R4 预算熔断重启绕过是安全边界问题故接轮末持久化（每轮仅写一次防高频）；R5 窗口比扫描表述窄（同 utterance 有互斥守卫）但 prefill 窗口+L883 槽位混用确凿故修；G5 read-file 白名单不用纯单值缓存（会破坏 settingsStore 持久化恢复与多 Agent 各自绑定模型两个合法场景，实码核查后改持久化目录集合方案）；R8-06 docker-compose tuner 无 build 段经验证员确认属已知设计取舍登记不修。
  - 未闭合项：SEC 安全簇 6 项延续用户裁决不动（局域网部署形态）；R8-06 登记不修（CXO-Tuner 无 Dockerfile，compose 注释已声明约定）；低优先级登记：physio/store.py update() 高频落盘仅原子化未节流、CompositionPanel 状态英文直出、PID 复用误杀残留窗口、directRequest 分支未同构防护、server/main.py:160 与 services/context_manager.py 同名 ContextManager 阅读混淆、MemorySearchRequest 越界 422 收紧属预期行为变化——全部显式登记于变更文档第五章。
  - 接续入口：后端需重启加载新代码；第六+七+八轮改动叠加在工作区未 commit（提交策略由人类定）；下次"继续下一轮"建议扫描焦点：R1-R5 五轮登记项复核、s0601 契约批次（累计 10 项）、s0602 技术债扫描（God 类/分层双轨等历史项）。

- **第七轮全面质量评估修复（整体）**（**已关闭**——人类终审批准收尾，2026-08-28；issue `模块0-20260828-03`，变更文档 `.trae/documents/20260828_模块0_第七轮全面质量评估修复.md`【已关闭+GN-004 双审记录在档：批次1 警示放行、批次2 PASS+人类 [V] 批准】；本条目为主线程统一收尾）
  - 工程过程：用户"继续下一轮"→ 动态基线（后端 4433 passed/0 failed + 前端 tsc 0 错误 + vitest 641/642，与第六轮收尾完全一致）→ 4 路 search 定位 + 2 路 general_purpose_task 深扫（后端深挖 14 项 + 前后端契约比对 11 项）→ 2 名独立交叉验证员逐条实码复核（后端 11 真+2 部分真+0 误报；契约 7 真+3 部分真+1 误报 D2 剔除——/api/audio/config 实际存在漏查；验证中额外发现 E16 backend_port global 缺失）→ 用户批准【修高+中 14 项，低优先级 11 项登记，SEC 延续登记不动】→ rules-6 先写文档 → 5 组并行修复（G1 WS防护 E1/E2/E3；G2 前端契约 D1/D5；G3 集群 E12/E13/E14/E15；G4 服务 E7/E8/E16；G5 原子写 E6/E11）→ 全量回归。
  - 交接状态：**14 项修复已闭合**（新增 25 例单测：后端 21 + 前端 4）；全量回归**已闭合**（后端 **4454 passed/0 failed**【基线 4433 净增 21 零回归，日志 round7_backend_final.log】；前端 tsc 0 错误、vitest **645/646**【净增 4 全过，唯一失败仍为既有 noble collector 存量，机器可读证据 round7_fe_vitest_final.json】）；**GN-004 交付前审查已闭合——警示放行（无 SOFT_BLOCK，2026-08-28）**：7 项 rubric 全通过（14/14 修复实码确认、三段交接完整、登记不动合规、证据链数字逐项一致、public/ 与 .trae/rules/ 零触碰、语义变化已披露、抽验 4 例测试断言非恒真）；2 项观察【均已收尾：①E1/E12/E13/E15 行号轻微漂移已在变更文档勘误校正 ②工作区多轮叠加未提交属已知状态，提交策略由人类定】；**人类 [V] 终审进行中**。
  - 最终结果：已爆发修复（D1 工具测试空参执行、E12 集群单 peer 队头阻塞丢事件、D5 live TTS 打断不生效）+ WS 防护（E1 畸形弹幕/E2 action 隔离/E3 畸形 JSON 跳帧，口径与 /ws/live 对齐）+ 集群健壮性（E13 假应用改拒 ack、E14 接管门槛只度量真实同步单元、E15 循环异常自愈）+ 服务管理（E7 锁内启动原子、E8 cwd 防误杀、E16 端口回读修正）+ 数据一致性（E6 avatars 原子写+锁、E11 图数据库关闭留痕+顺序修正）；修改面后端 8 文件+前端 2+测试 9（含新建 tools.test.ts）。
  - 为什么：畸形帧断连→跳帧对齐 /ws/live 既有防护口径（最小语义变化）；E13 拒 ack 而非补应用端（防假确认丢数据是 B4 不变量，应用端属功能开发非缺陷修复）；E7 用 RLock 因临界区内复用内部取锁辅助函数；E11 先 close 成功再移除注册表（close 失败引用可重试）。
  - 未闭合项：~~11 项低优先级登记~~ → **批次2 已全部修复闭合**（终审追加裁决全修，见接续入口）；SEC 安全簇 6 项延续用户裁决不动（局域网部署形态，§五显式登记）；pyi:119/admin-key UI/R1-R5 五轮登记延续。
  - 接续入口：~~主线程拉起 GN-004 交付前审查~~（已闭合，警示放行）→ 用户终审要求"先看低优先级"→ 追加裁决 11 项低优先级全修（D6 气泡标签/D9 轻量提示/D7D8 删前端死分支）→ **批次2 已闭合**（4 组并行：E4 半截回复入库+防双写、E9 孤儿回收、E10 lines 钳制、D3/D4/D10/D11 类型对齐、D6 voice.speaker 气泡标签、D7/D8 前端死分支删除、D9 toast+eventsStore+voice.interrupted 删发；新增 7 例测试，新建 toastStore/eventsStore/GlobalToast）→ 批次2 全量回归已闭合（后端 **4459 passed/0 failed**【4454 净增 5 零回归，round7_backend_final2.log】；前端 tsc 0 错、vitest **647/648**【净增 2 全过，round7_fe_vitest_final2.json】）→ **GN-004 复审批次2 已闭合——通过（PASS，2026-08-28）**：9 项 rubric 全通过、3 项观察已收尾（①E10/D4 行号勘误 ②E4 取消落 except 标记词错位登记知悉 ③tool_call 类型字段保留合理）→ **[V] 人类终局裁决：批准收尾 ✅（2026-08-28）**。收尾提醒：后端需重启加载新代码；第六+七轮改动叠加在工作区未 commit（提交策略由人类定）；2 个低优先级观察项（E4 标记词错位/tool_call 类型字段）与 SEC 安全簇在后续轮次可复核。

- **第六轮全面质量评估修复（整体）**（**已关闭**——人类终审批准收尾，2026-08-28；issue `模块0-20260828-02`，变更文档 `.trae/documents/20260828_模块0_第六轮全面质量评估修复.md`【已完成+GN-004 警示放行+[V]批准记录在档】；本条目为主线程统一收尾，吸收下方批次1-A 部分条目）
  - 工程过程：用户发起第六轮全面质量评估（性能/安全/兼容/扩展性/设计/Bug，许可不限 subagent）→ 动态基线（后端 4414 passed/0 failed + 前端 tsc 0 错误）→ 4 路并行深扫 + 1 路资源生命周期专项（35+7 项发现）→ 2 名独立交叉验证员复核（14 真/4 部分真/0 误报）→ 用户双裁决【SEC 安全簇 6 项登记不动；WS 协议断裂全部补实现】→ rules-6 先写文档 → 批次1（A 生命周期/B 性能/F 外围 3 并行）→ 批次2（C 设计收敛/D WS 主链路 2 并行）→ 批次3（live 七消息补实现）→ 全量回归 → GN-004 交付前审查。
  - 交接状态：**全部 6 批次已闭合**（24 项修复实施 + L2 免改 + 19 例新增单测）；全量回归**已闭合**（后端 4433 passed/0 failed【基线 4414 净增 19 零回归】；前端 tsc/eslint 0 错误、vitest 641/642 唯一失败为既有 noble collector 存量【机器可读证据 round6_fe_vitest_final.json】）；GN-004 **警示放行（无 SOFT_BLOCK）**已闭合（26 项抽验一致、public/ 与 .trae/rules/ 零触碰、3 观察项均已收尾：vitest JSON 证据重生成/note 统一收尾/行号勘误）；**[V] 人类终局裁决：批准收尾 ✅（2026-08-28）**。
  - 最终结果：性能（审计日志写放大+全局锁、300s LLM 占共享池、蒸馏缓存无界、热路径同步 IO/重复编译正则）、生命周期（WS 断连 finally 化 5 端点、日志句柄/解码器/MCP 子进程泄漏、lifespan 显式取消、autonomy 3 处原子写）、设计（agents.json 五入口收敛 AgentConfigStore、core→api 反向依赖消除、CoreException 全局 handler）、WS 协议补实现（alarm 自动订阅恢复提醒功能、打断事件、thinking 帧、voice 补 session_id、live 七消息含重建 live AI 回复链路）、外围（WorkStation 启动口径、tuner healthcheck、bat×3、admin client 契约）——修改面后端 25 文件+前端 1+外围 4，新建 agent_store.py/prompts_constants.py。
  - 为什么：CancelledError 不被 except Exception 捕获故断连清理必须 finally 化；审计日志契约锁定 .json 路径且 public/ 禁改故采用有界数组+分会话锁+原子写而非 JSONL；live 原无回复链路系"补出而非挂接"（与裁决一致且显式披露，降级六点核验安全）；agents.json 实测顶层扁平 list 而 agent_tools 期望 dict 的口径分裂以实际文件为准统一。
  - 未闭合项：SEC-1~6 安全簇（用户裁决本轮不动，§五显式登记）；God 类×2/分层双轨/autonomy 容器/R1-R5/pyi:119/admin-key UI/C7/E2E 三重闸不可判定（沿既有口径）——全部显式登记非静默；W3 打断事件仅接双流入口（半双工/live 两入口登记后续）。
  - 接续入口：后端重启加载新代码；全部改动在工作区未 commit（提交策略由人类定）；待人类 [V] 终审后本轮归档。

- **第六轮全面质量评估修复——批次1-A 生命周期 L1~L7**（已闭合，2026-08-28；issue `模块0-20260828-02`，变更文档 `.trae/documents/20260828_模块0_第六轮全面质量评估修复.md` §三批次1-A + §六批次1-A小节）
  - 工程过程：s0401 写前闸门判定（代码修补=免检通行区放行，锚点文档回填=格式合规放行）→ 实码逐条核对（发现 L2 描述的裸 create_task 实际已呈 `_track_background_task` 包裹形态，未重复改动）→ 7 文件串行编辑（同文件多处修改防覆盖串行执行）→ 定向 pytest 验证 → 变更文档批次1-A 回填。
  - 交接状态：**批次1-A 已闭合（7/7：L1/L3/L4/L5/L6/L7 实施完成，L2 经实码核验已达标免改）**；批次1-B/1-F、批次2/3 未闭合（归并行子代理与主线程）；本轮整体 GN-004 审查与人类裁决未做（待主线程拉起，本 subagent 上下文不可自行拉取）。
  - 最终结果：定向测试 **812 passed / 0 failed**（`-k "websocket or gateway or live or service or video or vision or mcp or meeting or token or lifespan or audio"`，3602 deselected，49.56s，exit 0）；改动清单与逐项行为见变更文档 §六批次1-A（websocket.py 4端点+gateway/server.py 主handler 断连清理 finally 化 / service.py `_close_backend_log_handle` 幂等释放接入三处 / video_understanding cv2/av try-finally 释放+mktemp→mkstemp / mcp.py stop/unload/close 超时 kill 升级+communicate(timeout=5) / token.py 超时任务异常兜底 / main.py lifespan shutdown 显式 cancel+gather）。
  - 为什么：asyncio.CancelledError 继承 BaseException 不被 except Exception 捕获，断连清理必须 finally 化；日志句柄/解码器/子进程/后台任务的生命周期终点必须与其所有权终点绑定释放；L2 实码已达目标形态证明评估文档行号/状态存在漂移，修复前逐条实码核对是必要闸门。
  - 未闭合项：批次1-B（P1~P6）、批次1-F（F1/X1~X4，其结果已先行回填变更文档 §六）、批次2（E1~E3/W1~W4）、批次3（W5）；全量 pytest 回归与前端闸门待全部批次完成后执行；GN-004 交付前审查 + 人类终审待主线程。
  - 接续入口：主线程合流批次1（1-B/1-F 完成后）→ 批次2 → 批次3 → 全量回归 → GN-004 审查 → 人类终审；全部改动在工作区未提交（遵守禁 git 提交约束），未触碰 public/ 与 .trae/rules/。
...
- **Qwen3 TTS 统一迁移 + CosyVoice3 主引擎架构**（已闭合，2026-08-16；spec `cosyvoice2-primary-qwen3tts-base-fallback`）
  ...
  - 最终结果：CosyVoice3-0.5B 主运行时（带 refs 克隆/情感，RTF≈0.47，流式首包 1.5s）+ VoiceDesign 保留（无 refs 音频设计，8091）+ Qwen3-TTS Base 降级（8093）+ IndexTTS 移除；契约 v1.5.0 落位 + v1.5.1 PATCH（CosyVoice3 默认值/描述同步，人类授权）；后端 TTS 定向 72 passed + 全量回归通过 + pyflakes 零警告。已知限制：flow-steps 5 为质量略降的标准做法；CosyVoice 流式 hop 边界可能存在 20ms 级拼接痕迹（O4 已披露为可接受项）。
  - 接续入口：c5d [V] AskUserQuestion 人类批准交付后，标记 Task 8 / checklist [V] 项闭合，spec 归档。
...
- **/goal 深度测试+架构/提示词优化**（进行中）
  - 代码洁癖：删除 hybrid_search 死回退分支 ✅（2026-08-13）：`server/core/memory/hybrid_search.py` 的 `_keyword_search` 曾有一 `try/except TypeError` 双分支，第一分支带 `agent_id`、except 分支不带。经核验 `crud_mixin.search_memories` 明确接受 `agent_id`（L179），且 `sqlite_manager` 即 `MemoryManager`，故 except 回退分支永不可达（per-agent 迁移期间的临时兼容代码残留），予以删除，仅保留带 `agent_id` 的单一调用。对照 `_vector_search` 的 except 回退**予以保留**——`chroma_store`/`milvus_lite_store` 的 `search_similar` 尚不接受 `agent_id`，回退仍可达。验证：pyflakes 零告警、定向 798 passed、全量回归 **2822 passed 零失败**（58.67s）。变更文档 `20260813_模块0_删除hybrid_search死回退分支.md`。
  - 代码洁癖：删除 api/exceptions.py 四个死异常子类 ✅（2026-08-13）：AST 全仓扫描（含别名引用）确认 `server/api/exceptions.py` 的 4 个 `ServiceError` 子类（`DatabaseError`/`LLMError`/`VectorStoreError`/`ValidationError`）全仓（server+tests）零引用——生产只 `raise ServiceError` 基类（app.py L103 注册 handler），`VectorStoreError` 被 core/exceptions 活跃同名类遮蔽，全局错误码契约 `public/schema/error_codes.schema.json` 为种子阶段空 schema 未声明 4 个错误码。经人类确认删除 4 个子类，仅保留 `ServiceError` 基类 + 4 个全局异常处理器。其余扫描候选（APIResponse/PaginatedResponse/HealthResponse/ErrorResponse ↔ api_response.schema.json、AgentConfig ↔ agent.schema.json、ChatResponse ↔ chat.pyi、get_handler_name/create_request ↔ interface_stub、get_service_url/get_async_memory_manager/start/stop_session_cleanup/reset_alarm_manager/set_graph_config/Neo4jImporter/LatencyTracker/ChromaVectorStore/MilvusLiteVectorStore/get_vad_processor/is_silence_pcm/generate_silence/PingMessage 等）均属契约模型或测试引用公共 API，予以保留。验证：pyflakes exceptions/response 零告警、定向 32 passed、全量回归 **2822 passed 零失败**（58.57s）。变更文档 `20260813_模块0_删除api_exceptions死异常子类.md`。
  - 文档完善：ACP 管线管理器 docstring 全量补齐 ✅（2026-08-12）：`core/acp/manager.py`（ACP 自动回复管线管理器）全部公开方法补齐中文 docstring（此前 AST 扫描 23 处缺失），覆盖 agent/connection/group CRUD、消息收发、统计与 per-agent 资源隔离方法；`ACPMessageInfo.to_dict` 等 4 个数据模型转 dict 方法亦补齐。验证：AST 复查缺失 `[]`、`pyflakes server/core/acp/manager.py` 零告警、ACP 相关测试 5 文件 104 passed 零失败。变更文档 `20260812_模块0_补齐公开类与函数docstring.md`（第五章追加）。
  - 架构从简+提示词工程：情感分析改用摘要模型 + 修复多模型配置 ✅（2026-08-12）：①`core/memory/emotion.py` 实现 `_analyze_with_llm`（调用注入的摘要模型客户端输出结构化 JSON polarity/intensity/emotion_type/confidence/keywords，失败回退规则词典）+ `_parse_llm_json`（容忍 markdown 块/包裹文本）；`get_emotion`/`get_emotion_for_decay` 改 async 并经 `analyze()` 走 LLM；`main.py` 摘要工具注册处 `set_emotion_llm_client(services.model_router.get_client("summary"))` 注入摘要模型。②修复多模型无法独立配置：`config.py::ModelsConfig` 新增 `_explicit` 集合 + `resolve_target()`（显式配置返回自身、否则按 `defaults` 跟随兜底），`get_model_config` 收敛为 `getattr(self, resolve_target(...), self.main)`，`Settings._load_config()` 装填后记录显式模型节；`model_router.py::get_client` 移除重复 defaults 跟随（客户端已在创建阶段按解析后配置生成）。③补测试覆盖：`test_emotion.py` 新增 LLM 模式 6 项（成功解析/越界钳制/异常回退/非法 JSON 回退/use_llm=False 忽略/set_llm_client 启用）+ `_parse_llm_json` 4 项；`test_config.py` 新增 `resolve_target`/`_set_explicit` 显式覆盖 3 项（explicit summary/memory 独立生效、未显式跟随 main、未记录 explicit 仍跟随 defaults）。验证：pyflakes 源/测试零告警、定向 87 passed、全量回归 **2841 passed 零失败**（净增 13 条测试）。变更文档 `20260812_模块0_情感分析改用摘要模型与多模型配置修复.md`。
  - 代码洁癖：删除 LLMFactory 冗余别名 get_client + 18 个零引用死类 ✅（2026-08-12）：①`LLMFactory.get_client` 为 `create_client` 纯别名（`return cls.create_client(...)`），全仓生产零调用、仅 test_get_client_delegates 引用，删除方法+测试。②AST 全仓扫描（server+全部测试）确认 18 个符号全库仅出现一次（定义处）即零引用死代码：`exceptions.py` 的 `MemoryNotFoundError`/`AgentNotFoundError`/`SessionNotFoundError`/`AuthenticationError`/`RateLimitError`（5 个，活跃体系只剩 DatabaseError/LLMError/VectorStoreError/ValidationError）；`acp.py` 的 `ACPGroupJoinRequest`/`ACPGroupLeaveRequest`（join/leave 路由用 path 参数非 body 模型）；`decision.py` 的 `RubricSnapshotModel`（实际用 decision_core.RubricSnapshot）；`graph.py` 的 `ShortestPathRequest`（shortest_path 路由用 query 参数）；`migration.py` 的 `MigrationManager`（Neo4jImporter/Exporter 有测试保留）；`decay.py` 的 `DecayParams`；`client.py` 的 `LLMMessage`；`mcp.py` 的 `MCPConnectionError`/`MCPTimeoutError`；`actions.py` 的 `ExternalEventsActions`；`graph_store.py` 的 `UserRelationType`/`ThingRelationType`/`ConceptRelationType`/`EventRelationType`（4 个枚举，ENTITY_TYPE_TO_LIBRARY 只用 EntityType）。删除后处理 `RubricSnapshotModel` 遗留的 decision.py 未用 `List` 导入。验证：pyflakes server 零告警、定向 529 passed、全量回归 **2828 passed 零失败**。变更文档 `20260812_模块0_删除LLMFactory冗余别名get_client.md`、`20260812_模块0_删除18个零引用死类.md`。
  - 代码洁癖：删除 13 处全仓零引用死函数/常量 ✅（2026-08-12）：用 AST 脚本扫描模块级函数与常量，排除 FastAPI 路由（含 `@router.xxx` 的 `ast.Call` 装饰器形态）、pydantic 装饰器、pytest 夹具后，确认并删除 13 个全仓零引用符号：`logging_config.get_logger`（残留薄封装）、`vector.get_vector_store()`、`decay.importance_to_score`、`decision_core.LOCATIONS`/`FINAL_ACTIONS`、`parser.SUPPORTED_DOC_MIMES`、`template_engine._DEFAULT_PRESETS_DIR`/`_DEFAULT_CUSTOM_DIR`、`distillation_service._DEFAULT_SESSION_DIR`/`_DEFAULT_LOG_DIR`/`_MULTIMODAL_SOURCE_TYPES`/`_TERMINAL_STATES`；并顺带清理仅被已删常量引用的孤立 `_DATA_DIR`。正确保留被 L218 类属性引用的 `_DEFAULT_TEMPLATES_DIR`（非死代码）。验证：pyflakes 7 文件零告警、定向 223 passed、全量回归 **2853 passed 零失败**（60.62s）。变更文档 `20260812_模块0_删除零引用死函数与常量.md`。
  - 代码洁癖：删除三处死私有方法 ✅（2026-08-12）：用临时 AST 脚本全仓扫描"定义但全仓零调用"的私有方法，并系统性排除语言调用（dunder `__str__` 等）、pydantic 装饰器（`@model_validator` 的 `_resolve_*`、`@field_serializer` 的 `_ser_*`）、pytest autouse 夹具（`_clean_settings`/`_clean_sessions`）等假阳性后，确认并删除三处真实死代码：①`core/decision/decision_core.py` 的 `_read_audit_logs`（注释标"仅供测试"却测试也零引用）；②`api/routers/multimodal.py` 的 `_reset_pipeline()`（标"测试用"却零调用）；③`api/routers/audio.py` 的 `_load_asr_config()`（全仓零调用）。验证：pyflakes 三文件零告警且无孤立 import、定向 141 passed、全量回归 **2853 passed 零失败**（60.27s）。变更文档 `20260812_模块0_删除三处死私有方法.md`。
  - 架构从简+代码洁癖：收敛 _iso_now/_new_uuid 重复辅助到共享 utils ✅（2026-08-12）：`_iso_now()`（`datetime.now(timezone.utc).isoformat()`）与 `_new_uuid()`（`str(uuid.uuid4())`）在 6 个模块逐字重复定义——`core/decision/decision_core.py`、`core/decision/agent_tools.py`、`core/distillation/distillation_service.py`、`core/memory/mixins/decision_mixin.py`、`core/multimodal/multimodal_pipeline.py`、`core/template_engine/template_engine.py`。`server/core/utils.py` 新增公共 `iso_now()`/`new_uuid()`；5 个真实使用的模块删本地定义改 `from server.core.utils import iso_now as _iso_now, new_uuid as _new_uuid`（调用点写法不变）；`agent_tools.py` 的两辅助定义在本模块从未调用（死代码），连同仅为它们服务的 `import uuid`/`datetime` 一并删除；各模块因删定义而不再使用的 `import uuid`/`datetime`/`timezone` 一并移除（保留仍被其它代码使用的）。另上一轮顺带收敛 `async_manager.py` 本地重复的 `json_dumps`/`json_loads` 至 `_common.py` 单一真相源。验证：pyflakes 对 8 个改动文件零告警、定向 236 passed、全量回归 **2853 passed 零失败**（62.32s）。变更文档 `20260812_模块0_收敛iso_now_new_uuid共享utils.md`、`20260812_模块0_收敛async_manager重复json辅助函数.md`。
  - 代码洁癖：删除死特性 ContextSummarizer ✅（2026-08-12）：`core/context/summarizer.py` 的 `ContextSummarizer`（规则式对话摘要：简洁/详细风格、提取关键点）全仓生产代码零调用、仅测试引用，却在 `core/context/__init__.py` 被导出；实际对话摘要走 **LLM 摘要客户端**（`model_router.get_client("summary")`，经 secondary_router/context 路由落地，有独立 summary 模型配置），该规则类是早期预留、项目改走 LLM 方案后搁置的死实现（参考项目 `C:\CXHMS` 同样仅导出从不实例化）。经人类确认「那就删吧」整特性删除：删 `summarizer.py`/`test_context_summarizer.py`、`context/__init__.py` 移除导出、`config.py::ContextLimitsConfig` 移除仅被该类消费的 `summarizer_max_topics`/`summarizer_max_key_points`、清理 `test_prompt_engineering_optimization.py` 的 `TestSummarizerExtractJson` 与 `AsyncMock`/`pytest` 未用导入、`config/default.yaml` 移除两字段、`docs/technical.md` 移除摘要器条目并更新限额行。验证：pyflakes 三文件零告警、定向 139 passed、全量回归 **2866 passed 零失败**（净减约 19 条死特性测试），全仓已无 `ContextSummarizer` 引用。变更文档 `20260812_模块0_删除死特性ContextSummarizer.md`。
  - 架构从简：ACP 自动回复收敛 build_messages 单入口 ✅（2026-08-11）：`core/acp/manager.py::_trigger_auto_reply` 原手动拼 `system_prompt + ACP_REPLY_HINT_PROMPT + history + incoming_message`，绕过 AGENTS.md §4.9 强制单入口。现 `ACP_REPLY_HINT_PROMPT` 与 `ACP_HISTORY_LIMIT=50` 收敛至 `prompt_builder.py` 唯一真相源；`build_messages` 新增 `acp_context` 参数 + ACP 模式分支（注入 ACP 提示 + 历史 + incoming_message，不追加 user 轮次、不注入主聊天隐藏提示词，与历史行为逐字等价）；`_trigger_auto_reply` 改调 `build_messages`，删除手动拼装与重复常量。工具调用循环沿用 `generate_chat_stream`（已收敛单入口）。验证：pyflakes 两文件零告警、ACP/提示词/chat_stream 109 passed、全量回归 **2929 passed 零失败**。变更文档 `20260811_模块0_ACP自动回复收敛build_messages单入口.md`。
  - 契约对齐：情感音色加载器与中性回退键 ✅（2026-08-11）：`tts_service._load_emotion_voices` 原为扁平扫描（与产出方 `emotion_ref_generator` 的子目录 + `emotion_mapping.json` 布局冲突，该布局下恒扫不到 → 情感合成静默回退默认音色），收敛为对唯一真相源 `tts_audio_utils.load_emotion_voices` 的薄封装；`get_emotion_voice` 中性回退同时识别 `neutral`（解析器 canonical）与 `normal`（历史别名），修复 `[emotion:neutral]` 无法命中中性音色缺陷。验证：全量回归 2925 passed 零失败。变更文档 `20260811_模块0_TTS情感音色加载器收敛.md`。
  - 提示词对齐解析器 ✅（2026-08-11）：`config/hidden_prompt.yaml` 的 `emotion_prompts` 全角 `【happy】` → 解析器契约 `[emotion:happy]`，情感名单对齐 `SUPPORTED_EMOTIONS` 全集 15 种；`effect_prompts` 全角 `（音效名）` → `[effect:name]`。修复解析器对新名单外标记告警并把标记当纯文本保留的漂移。另修复 `tts_service.synthesize_stream_with_emotions` 音效分支死条件 `type=="sound"`（解析器恒产出 `"effect"`）→ 音效 chunk 永不生成的 Bug。验证：全量回归 2922 passed 零失败。变更文档 `20260811_模块0_情感音效提示词对齐解析器.md`、`20260811_模块0_流式TTS音效分支类型失配修复.md`。
  - 代码洁癖：删除 context_manager.get_context_str 死代码 ✅（2026-08-12）：`context_manager.py::get_context_str` 全仓（生产+tests）零调用，系历史遗留的纯格式化辅助方法，删除。`get_context_with_system_prompt` 等仍被 `interrupt_llm` 生产使用，不受影响。验证：pyflakes 零告警、定向 43 passed、全量回归 **2920 passed 零失败**。变更文档 `20260812_模块0_删除context_manager死代码.md`。
  - 代码洁癖：删除标记适配残留死代码 ✅（2026-08-12）：`marker_adapter.get_supported_markers`/`_supported_markers` 与 `frontend_marker.register_marker`/`_markers` 全仓生产零调用（仅被自身测试引用），删除并同步移除 `test_marker_adapter.py::TestSupportedMarkers`、`test_frontend_marker.py::TestRegisterMarker` 与恒真的 `result is not marker._markers` 断言。验证：pyflakes 两文件零告警、定向 39 passed、全量回归 **2920 passed 零失败**（净减 2 死代码测试）。变更文档 `20260812_模块0_删除标记适配残留死代码.md`。
  - 架构从简+代码洁癖：frontend_marker format_for_frontend 接线上 markers 列表 ✅（2026-08-12，按用户"直接接线上"确认）：`format_for_frontend` 原只检查顶层 `emotion`/`effect` 键，但 `process_danmaku`/`process_message` 真实产出是 `markers` 列表，导致转换分支在真实链路永不触发、退化为 `data.copy()` 无操作。现接线上 markers 列表——逐条为 emotion/effect 标记追加 `frontend_emotion`/`frontend_effect`，新增 `_decorate_marker` 不污染原始 markers，保留历史兼容。验证：pyflakes 零告警、定向 41 passed、全量回归 **2922 passed 零失败**。变更文档 `20260812_模块0_前端标记接线上markers列表.md`。
  - 性能优化：frontend_marker 情感映射常量提升 ✅（2026-08-12）：`frontend_marker.py` 的 `_convert_emotion`/`_convert_effect` 每次调用重建映射 dict 并两次构建默认中性值，弹幕热路径不必要分配；提升为模块级常量 `_EMOTION_MAP`/`_EFFECT_MAP`/`_NEUTRAL_EMOTION`/`_CUSTOM_EFFECT`，方法改查表返回（行为逐字等价）。验证：pyflakes 零告警、定向 28 passed。变更文档 `20260812_模块0_前端标记映射常量提升.md`。
  - 性能优化：直播弹幕流热路径 gift/enter 日志降级 ✅（2026-08-12）：`live_client.py` 的 `_handle_gift`（`Gift received`）与 `_handle_enter`（`User entered`）为每事件 INFO 日志且 eager f-string 格式化整个 data，实录弹幕流下高频刷屏；降级 DEBUG 并改 `%` 惰性格式化，与 danmaku 过滤日志及热路径门控约定一致。验证：pyflakes 零告警、定向 18 passed。变更文档 `20260812_模块0_直播弹幕流热路径日志降级DEBUG.md`。
  - 架构从简+代码洁癖：收敛 marker_adapter 重复标记提取与删除休眠正则 ✅（2026-08-12）：`marker_adapter.py` 的 `process_danmaku`/`process_message` 各重复约 20 行「遍历 EMOTION/EFFECT_PATTERN 提取标记」逻辑，提取为私有 `_extract_markers(content)` 共享（行为逐字等价，ACTION 分支保留）；删除 `emotion_parser.py` 全仓零引用的 `SLEEP_PATTERN`（sleep 由 `COMBINED_PATTERN` 内部处理，系上轮删 `get_emotion_at_position` 后遗留）。验证：pyflakes 三文件零告警、定向 51 passed、全量回归 **2920 passed 零失败**。变更文档 `20260812_模块0_收敛marker_adapter标记提取与删除休眠正则.md`。
  - 代码洁癖：删除防火墙死状态 ✅（2026-08-12）：`server/services/firewall.py` 存在两个仅赋值从不读取的死状态——`_context_manager`（经 `set_context_manager` 注入）与 `_filter_callback`；`gateway/server.py` 启动路径为其孤立的仅注入上下文管理器调用点。删除 `_context_manager` 字段/`set_context_manager` 方法、`_filter_callback` 字段及不再使用的 `Any`/`Callable`/`Optional` typing 导入；`gateway/server.py` 删除 `FirewallService.get_instance()` 与 `get_context_manager` import（防火墙为懒加载单例，无提前实例化必要）；`test_firewall.py` 删除仅验证 setter 的 `test_set_context_manager_accepts`。注意：`interrupt_llm` 的 `_context_manager` 为真实读取方，本次未触碰。验证：pyflakes server 零告警、定向 firewall+live_client 42 passed、全量回归 **2906 passed 零失败**（净减 1 死代码测试）。变更文档 `20260812_模块0_删除防火墙死状态.md`。
  - 代码洁癖：删除过期系统提示词特性 ✅（2026-08-12）：`server/services/context_manager.py` 遗留整套「系统提示词」状态机（`system_prompts`/`system_prompt_sent`/`set_system_prompt`/`get_system_prompt`/`mark_system_prompt_as_sent`/`is_system_prompt_sent`/`get_context_with_system_prompt`），但 `set_system_prompt` 全仓**零真实调用**（唯一引用在 ASYNC_OPTIMIZATION_GUIDE.md 文档），`get_context_with_system_prompt` 因 `get_system_prompt` 恒返回 `""` 退化为 `get_context` 等价转发。删除整套特性并简化 `clear_context`，移除不再使用的 `logging`/`logger`；`interrupt_llm.py` 的 `_get_context_with_system_prompt` 内联为直接调 `get_context` 并重命名 `_get_context`；`test_interrupt_modules.py` mock 改 `get_context`；`ASYNC_OPTIMIZATION_GUIDE.md` 移除系统提示词章节与 Token 基准表、标题改「异步向量化优化」。验证：pyflakes 两文件零告警、定向 93 passed、全量回归 **2918 passed 零失败**。变更文档 `20260812_模块0_删除过期系统提示词特性.md`。
  - 代码洁癖：删除 adaptive_polling 死特性 ✅（2026-08-12）：`adaptive_polling.py`（自适应轮询管理器）全仓生产代码**从不读取**计算出的间隔——`get_poll_interval()` 仅测试调用、`init_adaptive_polling_manager` 生产零调用（配置从未注入运行时）、`live_client.py` 的 `record_packet()` 是纯空操作（记录延迟数据无人消费）。经人类确认「整体删除整个特性」后：删除 `server/services/adaptive_polling.py` 模块、`live_client.py` 的 import/实例/record_packet no-op、`server/config.py` 的 `AdaptivePollingConfig` 模型与字段、config 路由的 schema/默认配置/`GET|POST /config/adaptive-polling` 端点/两处处理块、`server/config/settings.json` 与 `c:/CX-O/config/settings.json` 配置段、前端 `config.ts` 死方法 `getAdaptivePollingConfig()`、`docs/technical.md` 4.9 条目、`tests/test_adaptive_polling.py` 整个文件、`tests/test_live_client.py` 的 FakePollingManager/packets 断言、`tests/test_config_router.py` 的 `TestAdaptivePolling` 类；`public/config_template/` 两处注释（已获人类显式授权）。验证：pyflakes 三文件零告警、定向 43 passed、全量回归 **exit 0 零失败**（净减 21+2 条测试）。变更文档 `20260812_模块0_删除adaptive_polling死特性.md`。
  - 代码洁癖：删除 ModelRouter 四个死方法 ✅（2026-08-12）：`ModelRouter.chat`/`get_embedding`/`is_available`/`get_all_status` 全仓生产零调用（业务已改用 `get_client` 直接取客户端，仅测试引用）。经人类确认后删除 4 方法并移除 `typing.List` 导入；同步删除 `tests/test_model_router.py` 的 `TestChat`（4 条）、`TestGetEmbedding`（3 条）、`TestStatus` 的 2 个死用例，简化 `_fake_client`（移除 chat/embedding 参数与 get_embedding 方法），更新模块 docstring。验证：pyflakes 两文件零告警、定向 17 passed、全量回归 **2887 passed 零失败**（净减 9 条测试）。变更文档 `20260812_模块0_删除ModelRouter死方法.md`。
  - 代码洁癖+bug修复 ✅（2026-08-12）：①`main.py` 的 `_shutdown_backup` 调用 `BackupManager.shutdown()`，但该占位类**无 shutdown 方法**，每次关闭都产生误导性「BackupManager关闭失败」警告（被 shutdown_service 捕获）；因 BackupManager 无资源可关，删除该无意义关闭块。②`BackupManager.export_backup` 仅测试引用、路由无导出端点，为死方法，删除并删对应测试。③顺带清理 `test_backup.py` 的 `import pytest` 未用与 `sunny` 未用变量。验证：pyflakes 对 `server/` 全目录零告警、定向 41 passed、全量回归 **2886 passed 零失败**。变更文档 `20260812_模块0_修复backup关闭bug删死方法.md`。
  - 代码洁癖：删除 core/exceptions.py 四个死异常类 ✅（2026-08-12）：`core/exceptions.py` 的 `DatabaseError`/`ValidationError`/`LLMError`/`ContextError` 全仓生产零调用、仅测试引用，且其中 3 个与活跃体系 `api/exceptions.py` 的真实异常类同名碰撞（项目存在两套并行异常体系：活跃的 `ServiceError` 体系 + 旁系的 `CoreException` 体系，后者仅 ACPError/MemoryOperationError/VectorStoreError/MCPError/ToolError 被生产使用）。经人类确认删除 4 个死类，`test_exceptions.py` 的 `TestSubclasses` 用例改用保留的 5 个类。验证：pyflakes 两文件零告警、定向 21 passed、全量回归 **2882 passed 零失败**（净减 4 条测试）。变更文档 `20260812_模块0_删除core_exceptions死异常类.md`。
  - 代码洁癖+测试修复 ✅（2026-08-12）：①`core/cache.py` 的 `cached()` 装饰器、`CacheManager.get_all_stats()`/`clear_all()` 三个成员全仓生产零调用、仅测试引用（`agent_config_cache` 单例被 `api/routers/agents.py` 正常使用，属保留），经人类确认删除并移除 `Callable`/`wraps` 导入，同步删 `test_cache.py` 的 `TestCachedDecorator`/`test_get_all_stats`/`test_clear_all` 与 `import pytest`。②全量回归暴露 `test_distillation_service.py::test_finalize_with_agent_creation_created` 未 `await` 已是协程的 `finalize_with_agent_creation`（TypeError + coroutine never awaited），为既有测试 bug，补 `@pytest.mark.asyncio` + `await`。验证：pyflakes 零告警、定向 12+47 passed、全量回归 **2886 passed 零失败**（首轮 1 failed → 修复后 0）。变更文档 `20260812_模块0_删cache死成员修distillation协程.md`。
  - 代码洁癖：删除 logging_config 死函数 get_logger ✅（2026-08-12）：`core/logging_config.py` 的 `get_logger()`（`logging.getLogger` 薄封装）全仓生产零调用、仅测试引用——所有生产日志入口统一用 `get_contextual_logger()`（ContextualLogger 门面），普通 `get_logger` 从未被生产模块导入。经人类确认删除，同步删 `test_logging_config.py` 的 `test_get_logger_returns_named_logger` 与导入，并顺带清理该文件不再使用的 `import pytest`。验证：pyflakes 两文件零告警、定向 22 passed、全量回归 **2885 passed 零失败**（净减 1 条测试）。变更文档 `20260812_模块0_删除logging_config死函数get_logger.md`。
  - 代码洁癖+架构从简：删除孤立死私有方法与未接线 API 聚合 ✅（2026-08-12）：①`multimodal_pipeline.py` 删 `_ocr_worker`/`_vision_worker`/`_merge_ocr_vision` 三个死包装（真实管线 `_image_worker` 已直接委托 `_image_worker_impl`，三者全仓零调用）；②`decision_core.py` 删 `_llm_decide`（`_llm_call`+`_parse_llm_output` 的薄包装，主流程 L283/L380 直接调底层，零调用），清理孤立 `Tuple` import；③`db_mixin.py` 删空实现且全仓零调用的 `_release_connection`；④`graph_tools.py` 删生产零调用的私有 helper `_get_library`（图工具直接用 L364-367 元组映射）+ 同步删 `test_get_library_mapping`；⑤`performance.py` 删未接线的 `api_stats`/`record_api_call`/`get_api_stats` 聚合（`PerformanceMiddleware.dispatch` 只记日志与响应头、从未调用聚合，且无端点消费 `get_api_stats`），删 `test_api_response.py` 的 `TestPerformanceStats` 与 import。已确认 `get_handler_name`/`create_request` 属 `public/interface_stub` 契约**予以保留**。验证：pyflakes 六文件零告警、定向 172+18+34 passed、全量回归 **2848 passed 零失败**（净减 19 条孤儿测试）。变更文档 `20260812_模块0_删除孤立死私有方法与未接线聚合.md`。
  - 文档完善：补全 LLM 客户端与嵌入 docstring ✅（2026-08-11）：全 server 扫描确认仅 `server/core/llm/client.py`、`server/core/memory/embedding.py` 两个真实模块缺模块级 docstring（其余 18 个为 `__init__.py` 包标记，按惯例可空），已补齐并说明连接池复用约定。验证：pyflakes 零告警、定向 46 passed。变更文档 `20260811_模块0_补全LLM客户端与嵌入docstring.md`。
  - 代码洁癖：删除 handlers 包死聚合 register_handlers ✅（2026-08-12）：`handlers/__init__.py::register_handlers` 全仓（生产+tests）零调用——真实接线在 `gateway/server.py` L162-182 逐条调用各 `register_*_handlers`，与该聚合调用清单完全重复；且聚合带设计缺陷（`asr_service/tts_service is None` 时早退会跳过全部处理器注册），故 gateway 未采用。删除该死聚合与仅其签名使用的 `TYPE_CHECKING` import 块，包文件退化为 docstring。全仓 Grep 确认无生产/测试/契约/docs 引用。验证：pyflakes 两文件零告警、9 个 gateway handler 定向 **88 passed**。变更文档 `20260812_模块0_删除handlers包死聚合register_handlers.md`。
  - 架构从简+代码洁癖：删除未接线的 server/storage/database 整包 ✅（2026-08-12）：`server/storage/database/`（connection.py 的 `AsyncConnectionPool`/`SyncConnectionPool` + migrations.py 的 `run_migrations` + `__init__.py`）生产全仓零引用——早期"统一连接池+集中式 schema 迁移"设计从未被采用，生产各数据模块（memory async_manager、context manager、session store、graph database、alarm manager、document memory、archiver）均各自维护 `_get_connection()`/`_init_db()` 并执行 `CREATE TABLE IF NOT EXISTS`，无一 import `server.storage`。不在 `public/interface_stub` 契约。经人类确认删除整个 `server/storage/` 包 + `test_connection_pool.py`/`test_migrations.py` 两个自测。验证：全仓 Grep 无残留、pyflakes server 零告警、全量回归 **2832 passed 零失败**（净减 15 条孤儿测试）。变更文档 `20260812_模块0_删除未接线storage数据库包.md`。
  - 代码洁癖：删除 summary_tools 死配置函数 update_topic_summary_config ✅（2026-08-12）：`summary_tools.py::update_topic_summary_config(key,value)` 全仓生产零调用、仅其自测引用——话题摘要配置的写入实际走 `set_max_history_topics`（被 master/summary 双层注册且有真实工具使用），`update_topic_summary_config` 是未被任何工具/路由挂接的旁路死代码。删除该函数 + `test_update_known_key`/`test_update_unknown_key` 两个自测。`_TOPIC_SUMMARY_CONFIG` 字典与读取路径（`get_topic_summary_config`/`max_history_topics`）保留。另核验 `get_async_memory_manager`/`get_decay_batch_processor` DI getter 访问 main.py 实际实例化的生产服务、属统一测试的 DI 存取器**予以保留**。验证：pyflakes 两文件零告警、定向 88 passed、全量回归 **2830 passed 零失败**（净减 2 条孤儿测试）。变更文档 `20260812_模块0_删除summary_tools死配置函数.md`。
  - 架构从简：收敛重复工具注册到 summary_tools ✅（2026-08-12）：`trigger_topic_summary`/`get_topic_summary_config`/`set_max_history_topics` 在 master_tools 与 summary_tools 双注册（元数据字节一致、函数等价，注册顺序决定生效函数——master 先跑故 wrapper 生效、summary 名存实亡）。经人类确认收敛到 summary_tools 为唯一注册源：删 master_tools 3 个重复注册块 + 3 个本地 wrapper + 空引用 import，删 test_master_tools 的 test_wrappers 与空 import asyncio。行为不变（wrapper 与 summary 直注册函数等价）。验证：pyflakes 零告警、工具定向 173 passed、全量回归 **2829 passed 零失败**（净减 1 条孤儿测试）。变更文档 `20260812_模块0_收敛重复工具注册.md`。
  - 代码洁癖：删除 summary_tools 死表达式 ✅（2026-08-12）：`trigger_topic_summary` 内 `datetime.now().strftime("%Y%m%d%H%M")` 为一棵值被丢弃的纯函数调用表达式（结果未用、无副作用），删除；局部 `from datetime import datetime` 保留（`summarized_at` 仍用 `isoformat()`）。另全仓 Grep 扫「纯函数调用结果丢弃」反模式仅 mcp.py 一处合法调用，无其他命中；核验 `config.py::get_service_url` 为合法薄公共配置辅助（读 `config.services[name].url`），予以保留。验证：pyflakes 零告警、定向 87 passed、全量回归 **2829 passed 零失败**。变更文档 `20260812_模块0_删除summary_tools死表达式.md`。
  - 代码洁癖：删除 audio.py 死函数 _load_asr_config ✅（2026-08-12）：AST 扫全仓模块级私有函数/常量仅 3 命中（__all__/__version__ 合法 dunder + `_load_asr_config`）；`audio.py::_load_asr_config()` 全仓生产零调用（仅定义处 1 引用），删整个函数块。`json`/`_PROJECT_ROOT`/`logger` 仍被他处使用无 import 孤儿。另核验工具注册无其他重叠（81 个工具名零重复，前期收敛已彻底）。验证：pyflakes 零告警、audio/tts 定向 46 passed、全量回归 **2829 passed 零失败**。变更文档 `20260812_模块0_删除audio死函数_load_asr_config.md`。
  - 代码洁癖：删除 decay.py 死函数 importance_to_score ✅（2026-08-12）：全仓公共模块函数扫描（AST 排除路由/pydantic/dunder）仅 `importance_to_score` 为真死代码——它是 `score_to_importance` 的反向映射（importance→score），但生产只消费 `score_to_importance`（decay_batch.py L110-112 将衰减 score 转回 importance），正向映射零调用。删除，保留 `score_to_importance`。另核验其余候选（get_decay_batch_processor/get_graph_database_if_exists/get_graph_store_if_exists/get_tts_sync_broadcaster/reset_alarm_manager/get_vectorization_queue/init_vectorization_queue/start_stop_session_cleanup）均被 tests 引用，属合法公共 API。验证：pyflakes decay/decay_batch 零告警、decay/batch 定向 32 passed、全量回归 **2829 passed 零失败**。变更文档 `20260812_模块0_删除decay死函数importance_to_score.md`。
  - 代码洁癖：清理 APP-Frontend 一次性调试临时脚本 ✅（2026-08-12）：删除 10 个自声明的临时调试文件（temp_cdp.cjs/temp_console.cjs/temp_file.cjs/temp_mock_backend.mjs/temp_probe.cjs/temp_scan.cjs/temp_shot.cjs/temp_test_out.txt/temp_text.cjs/temp_titles.cjs）——头部均声明"临时/非产品代码"，仅自身引用、无产品代码 import。另核验 docs 完整性：AST 扫 server 190 个模块 **0 缺 docstring**；ACP 自动回复热路径（`_trigger_auto_reply`）已充分优化（to_thread 预加载历史、build_messages 单入口、_save_data 批量），无冗余。验证：全仓 Grep 无残留、`npx tsc --noEmit` 零错误。`distillation_e2e_report_*.md`/`full_coverage.txt` 等历史报告按人类裁决保留。变更文档 `20260812_模块0_清理前端临时调试脚本.md`。
  - 代码洁癖：删除 embedding.py 冗余别名 get_model ✅（2026-08-12）：类内私有/特殊方法扫描（AST 排除 pydantic property/field_serializer/model_validator/staticmethod）仅 5 候选（`_resolve_absolute_paths`/`_ser_loaded`/`_ser_created`/`__lt__`/`__post_init__`）——全部为 pydantic/dataclass 运行时钩子，**无真死代码**。`EmbeddingFactory.get_model` 为 `create` 的纯别名（`return cls.create(...)`），全仓零调用（生产走 main.py L271 `create`），不在契约，删除。`clear_cache`/`list_available_providers` 被测试引用（test_embedding.py L123/L132），保留。验证：pyflakes 零告警、embedding 定向 14 passed、全量回归 **2829 passed 零失败**。变更文档 `20260812_模块0_删除embedding冗余别名get_model.md`。
  - 代码洁癖：删除前端未接线蒸馏客户端 ✅（2026-08-11）：`APP-Frontend/src/api/clients/distillation.ts` 的 `distillationApi` 全前端零导入（无页面/barrel/测试引用），删除并同步更新 `types.ts` 注释。**后端蒸馏服务（`server/core/distillation/` + `api/routers/distillation.py`，挂载 `/api/v1/distillation/*`）完整保留不受影响**。验证：`tsc --noEmit` 通过、vitest 36 文件 300 用例全 PASS。变更文档 `20260811_模块0_删除前端未接线蒸馏客户端.md`。
  - 代码洁癖：删除 emotion_parser 预留死代码 ✅（2026-08-11）：`strip_avatar_tags`/`strip_emotion_tags`/`get_emotion_at_position` 及 `AVATAR_TAG_PATTERN` 生产零调用、仅测试引用（疑似 avatar 动画预留），按用户确认删除 + 13 条对应测试。验证：pyflakes 零告警、全量回归 **2920 passed 零失败**。变更文档 `20260811_模块0_删除emotion_parser预留死代码.md`。
...
- 功能修复 ✅（2026-08-12：情感分析改用摘要模型 + 修复多模型配置）：① **情感分析用摘要模型**——`core/memory/emotion.py` 的 `_analyze_with_llm` 原为占位未实现（引用即 AttributeError），生产路径 `get_emotion_for_decay`（记忆写入）走同步规则词典；现实现 `_analyze_with_llm`（调 `llm_client.chat` 输出结构化 JSON：polarity/intensity/emotion_type/confidence/keywords，容忍 markdown 包裹，失败回退规则词典），`get_emotion_score`/`get_intensity_for_decay` 改 async 经 `analyze()` 走 LLM，模块函数 `get_emotion`/`get_emotion_for_decay` 改 async wrapper，新增 `set_emotion_llm_client()` 于 `main.py` 摘要工具注册处注入摘要模型客户端；`api/routers/memory.py` 改 await。② **修复多模型配置**——`ModelsConfig.defaults={"summary":"main","memory":"main"}` 在 `get_model_config` 与 `ModelRouter.get_client` 两处**无条件**跟随，把显式配置的不同模型也吞并；改为「显式配置优先、defaults 跟随兜底」：`ModelsConfig` 增私有 `_explicit` 集合 + `resolve_target()`，`_load_config` 注入显式模型节，`get_client` 移除重复跟随（客户端已在创建阶段按解析后配置生成），`get_model_info` 改用 `resolve_target`。验证：pyflakes 相关文件零告警、定向 254 passed、全量回归 **2828 passed 零失败**；手工验证显式 summary 不同模型返回自身节、未显式仍跟随 main、LLM 情感三路径（正常/markdown/回退）均正确。变更文档 `20260812_模块0_情感分析改用摘要模型与多模型配置修复.md`。
- 文档完善 ✅（2026-08-12：核心域类公开 docstring 补齐）：AST 扫描 `server/` 复扫结果——模块级 docstring 已 0 缺失，但仍有 105 公开类/472 公开函数缺 docstring。本轮聚焦核心业务域（记忆/图谱/缓存/CXFC/ACP/告警/备份）的**公开类**，为 19 个文件补充有实际含义的中文 docstring（1-3 句描述职责与关键行为，拒绝模板套话；仅补文档不改逻辑）。验证：AST 复扫目标文件公开类缺 docstring 数 = **0**、pyflakes `server/` 零告警、定向 275 passed、全量回归 **2828 passed 零失败**。变更文档 `20260812_模块0_核心域类公开docstring补齐.md`。
...
### 外部复审双问题存在性验证（2026-08-29，L1 静默记录）

- **做到哪了**：外部复审 2 项 Issue 逐条实码验证完毕，结论均为"问题不存在"，零代码改动；验证留痕 `.trae/documents/20260829_模块0_复审两问题存在性验证.md`（模块0-20260829-04，状态已完成）。
- **为什么**：Issue1 误报根因=评审按调用语法 `_count_active_memories(` 检索，漏掉 bare-reference 传参形态（memory.py:1105 `run_io(_count_active_memories, memory_mgr)`，全库恰一处定义+一处引用）；Issue2 误报根因=对 toolBridge.ts try 块重构区缩进误判，字节级实测 `}`@96=6 空格与外层 `try`@82=6 空格完全对齐，缩进阶梯 2/4/6/8 全链一致。
- **未闭合项**：原始 Diff 未随消息附上——若人类后续补发 Diff 且指向 toolBridge.ts 之外文件，Issue2 需按新 Diff 重验；Issue1 结论不受影响。
- **接续入口**：下轮复审前将两误报证据反馈评审源；无代码/测试动作。
