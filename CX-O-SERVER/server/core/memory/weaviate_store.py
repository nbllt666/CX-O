...
class WeaviateVectorStore:
        ...
        try:
            ...
            if self.embedded:
                ...
                self._client = weaviate.connect_to_local(
                    ...
                    headers=headers,
                    # 显式启用初始化检查：gRPC 不可用时直接抛错，
                    # 禁止静默降级为纯 HTTP 数据面（本项目要求数据面走 gRPC）
                ...
                logger.info(
                    f"Weaviate 客户端已连接: http={self.host}:{self.port} grpc={self.host}:{self.grpc_port}"
                )
    ...
    async def get_vector_by_id(self, memory_id: int, agent_id: str = "default") -> Optional[Dict]:
        ...
        try:
            ...
            if result.objects:
                ...
                # weaviate-client v4 的 obj.vector 为 named-vector 字典
                # （{"default": [...]}），需解包为扁平 List[float]
                vector_data = obj.vector
                if isinstance(vector_data, dict):
                    vector_data = vector_data.get("default") or next(
                        iter(vector_data.values()), []
                    )
                return {
                    "memory_id": obj.properties.get("memory_id"),
                    "content": obj.properties.get("content"),
    ...
    async def list_vectors(
        ...
        """分页列出向量（供向量数据管理 API 使用）

        直接从 per-agent collection 的 properties 读取元数据（content/memory_type/
        importance/created_at），无需回查 SQLite，避免 N+1 查询。
        数据面经 weaviate-client v4 走 gRPC。
        """
        if not self._client:
            return []
        ...
        try:
            ...
            query_kwargs: Dict = {"limit": limit, "offset": offset}
            if memory_type:
                from weaviate.classes.query import Filter

                query_kwargs["filters"] = Filter.by_property("memory_type").equal(memory_type)

            result = collection.query.fetch_objects(**query_kwargs)
            ...
            for obj in result.objects:
                ...
                vectors.append({
                    ...
                    "memory_type": obj.properties.get("memory_type", "unknown"),
            ...
            logger.error(f"Weaviate 列出向量失败: {e}")
            return []
    ...
    def get_collection_info(self, agent_id: str = "default") -> Dict:
        ...
        try:
            ...
            return {
                ...
                "agent_id": agent_id,
                # 契约对齐：向量管理路由消费 count 键（与 chroma/milvus store 一致）；
                # 保留 vectors_count 键兼容 memory.py 状态接口的消费
                "count": count,
                "vectors_count": count,
