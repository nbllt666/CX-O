...
from server.core.websocket import get_websocket_manager, get_chat_handler
...
async def lifespan(app: FastAPI):
    ...
    if not _has_settings:
        logger.warning("config.settings not available, skipping service initialization")
        yield
        return
...
def register_api_routes(app: FastAPI):
    ...
    if _has_settings and getattr(settings.config, "cors", None) and settings.config.cors.enabled:
        app.add_middleware(
            CORSMiddleware,
