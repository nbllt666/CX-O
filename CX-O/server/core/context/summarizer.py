...
class ContextSummarizer:
    ...
    async def summarize(self, messages: List[Dict], max_length: int = 500, style: str = "concise") -> Dict[str, any]:
        if not messages:
...
3. 保持对话的核心内容和意图
请以JSON格式返回：
{{"summary": "对话摘要", "key_points": ["要点1", "要点2", "要点3"]}}"""
            response = await self.llm_client.chat(messages=[{"role": "user", "content": prompt}], stream=False)
            result = json.loads(response)
            return {"summary": result.get("summary", ""), "key_points": result.get("key_points", []), "success": True}
        except Exception as e:
            logger.error(f"LLM摘要生成失败: {e}")
            return self._rule_based_summary(messages, max_length, style)

    def _rule_based_summary(self, messages: List[Dict], max_length: int = 500, style: str = "concise") -> Dict[str, any]:
        if not messages:
            return {"summary": "", "key_points": [], "success": True}
