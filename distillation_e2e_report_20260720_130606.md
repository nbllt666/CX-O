...
| 22 | quality_score 应非 None（S_STORAGE_DECISION 后必填），实际 0.8 | PASS |
| 23 | quality_score 应为 float 0.0-1.0，实际 0.8（type=float） | PASS |
| 24 | 状态应转移至 S_REJECT 或 S_FINALIZE，实际 S_FINALIZE | PASS |
| 25 | S_FINALIZE 时 quality_score 应 >= 0.3（reject_threshold 默认值），实际 0.8（若 < 0.3 应走 S_REJECT） | PASS |
